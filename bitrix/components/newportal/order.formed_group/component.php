...
$componentPage = '';
$arComponentVariables = array('formed_group_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_FORMED_GROUP_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_FORMED_GROUP_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_FORMED_GROUP_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['formed_group_id'];
    ...
    $arResult['PATH_TO_FORMED_GROUP_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_FORMED_GROUP_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[formed_group_id]=#formed_group_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'FORMED_GROUP';
