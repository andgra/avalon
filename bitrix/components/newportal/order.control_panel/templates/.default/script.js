if(typeof(BX.OrderControlPanel) === "undefined")
{
	BX.OrderControlPanel = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.OrderControlPanel.prototype =
	{
		initialize: function(id, settings)
		{
			...
			for(var i = 0; i < itemInfos.length; i++)
			{
				...
				var itemSettings = BX.OrderParamBag.create(BX.clone(itemInfo));
				itemSettings.setParam("panel", this);
				var item = BX.OrderControlPanelItem.create(itemSettings.getParam("id"), itemSettings);
				if(item.isActive())
				{
			...
			{
				var additionalItemSettings = BX.OrderParamBag.create(BX.clone(additionalItemInfo));
				additionalItemSettings.setParam("panel", this);
				this._additionalItem = BX.OrderControlPanelItem.create(additionalItemSettings.getParam("id"), additionalItemSettings);
			}

			this._searchButton = BX.findChild(this._container, { "tag": "SPAN", "class": "order-search-btn" }, true, false);
			if(this._searchButton)
			{
				BX.bind(this._searchButton, "click", BX.delegate(this._onSearchButtonClick, this));
			}

			this._pinButton = BX.findChild(this._container, { "tag": "SPAN", "class": "order-menu-fixed-btn" }, true, false);
			if(this._pinButton)
			{
			...
			this._slider = BX.OrderControlPanelSlider.create(
				this._id,
		...
		{
			return this.getSetting("itemContainerPrefix", "order_ctrl_panel_item_") + id.toLowerCase();
		},
		...
		adjust: function()
		{
			if(!this._isFixed)
			{
				...
				{
					this._isFixedLayout = false;
					BX.removeClass(this._wrapper, "order-menu-fixed");
			...
			if (this.getNodeRect(this._container).top <= 0)
			{
				...
				else
				{
					...
					BX.addClass(this._wrapper, "order-menu-fixed");
		...
		setFixed: function(fixed)
		{
			...
			BX.onCustomEvent(
				window,
				"OrderControlPanelFixed",
				[this, this._isFixed]
			);

			BX.userOptions.save("order.control.panel", this.getId().toLowerCase(), "fixed", (fixed ? "Y" : "N"));
		},
		_onSearchButtonClick: function(e)
		{
			var searcForm = BX.findChild(this._container, { "tag": "FORM", "class": "order-search" }, true, false);
			if(searcForm)
			{
				searcForm.submit();
			}
		...
		updateCounters: function(counters, send)
		{
			...
			for (var id in counters)
			{
				if (id == "ORDER_**")
				{
					var counter = BX("order_menu_counter", true);
					if (!counter)
					{
						break;
					}
	...
	BX.OrderControlPanel._default = null;
	BX.OrderControlPanel.setDefault = function(item)
	{
		this._default = item;
	};
	BX.OrderControlPanel.getDefault = function()
	{
		return this._default;
	};
	BX.OrderControlPanel.items = {};
	BX.OrderControlPanel.create = function(id, settings)
	{
		var self = new BX.OrderControlPanel();
...
if(typeof(BX.OrderControlPanelItem) === "undefined")
{
	BX.OrderControlPanelItem = function()
	{
		this._id = "";
		this._settings = null;
		this._panel = null;
	...
	BX.OrderControlPanelItem.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "";
			...
			if(!this._container)
			{
				throw "BX.OrderControlPanelItem: Container is not found.";
			}
			...
			if(this._findAction("CREATE"))
			{
				...
				this._container.appendChild(
					BX.create(
						"A",
						{
							"attrs": { "class": "order-menu-plus-btn" },
		...
		setActive: function(active, force, silent)
		{
			...
			if(active)
			{
				BX.addClass(this._getLink(), "order-menu-item-active");
			}
			else
			{
				BX.removeClass(this._getLink(), "order-menu-item-active");
			}
		...
		_getLink: function()
		{
			return BX.findChild(this._container, { "tag": "A", "class": "order-menu-item" }, true, false);
		},
		_onClick: function(e)
		{
	...
	BX.OrderControlPanelItem.items = {};
	BX.OrderControlPanelItem.create = function(id, settings)
	{
		var self = new BX.OrderControlPanelItem();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.OrderControlPanelSlider) === "undefined")
{
	BX.OrderControlPanelSlider = function()
	{
		this._id = "";
		this._settings = null;
		this._wrapper = null;
	...
	BX.OrderControlPanelSlider.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "";
			...
			if(!this._wrapper)
			{
				throw "BX.OrderControlPanelSlider. Wrapper is not found.";
			}
			this._items = this.getSetting("items");
			if(!this._items)
			{
				throw "BX.OrderControlPanelSlider. Items are not found.";
			}
			this._additionalItem = this.getSetting("additionalItem");
			this._anchor = BX(this.getSetting("anchorId"));


			if(typeof(BX.OrderControlPanelSliderInitData[this._id]) !== "undefined"
				&& typeof(BX.OrderControlPanelSliderInitData[this._id]["borderingItemIndex"]) !== "undefined")
			{
				this._borderingItemIndex = BX.OrderControlPanelSliderInitData[this._id]["borderingItemIndex"];
	...
	BX.OrderControlPanelSlider.create = function(id, settings)
	{
		var self = new BX.OrderControlPanelSlider();
		self.initialize(id, settings);
		return self;
	};
}
