...
function OrderPermRemoveRow(row)
{
	var roleId = row.getAttribute("data-roleId");
	if(BX.type.isNotEmptyString(roleId))
	{
		BX.Access.DeleteSelected(roleId, "orderPerms");
	}
...
function OrderSelectEntityInit()
{
	...
	BX.addCustomEvent(BX.Access, 'onSelectProvider', OrderPermAccessSelectProvider);
}
...
function OrderPermAddRow(obSelected)
{
	...
	{	
		...
		{
			var el = BX('orderPermTableInsertTd');
...
function OrderRoleDelete(title, message, btnTitle, path)
{
	var dlg = new BX.PopupWindow(
		'ORDER_ROLE_DELETE_' + Math.random().toString().substring(2),
		null,
		{
			autoHide: false,
			draggable: true,
...
function _OrderPermDisableProvider(providerId)
{
...
function OrderPermAccessSelectProvider(params)
{
	if(!(BX.Access && arOrderPermSettings && BX.type.isArray(arOrderPermSettings['DISABLED_PROVIDERS']) && arOrderPermSettings['DISABLED_PROVIDERS'].length > 0))
	{
		return;
	}

	var curProviderId = BX.type.isNotEmptyString(params['provider']) ? params['provider'] : BX.Access.selectedProvider;
	for(var i = 0; i < arOrderPermSettings['DISABLED_PROVIDERS'].length; i++)
	{
		var providerId = arOrderPermSettings['DISABLED_PROVIDERS'][i];
		if(providerId === curProviderId)
		{
			BX.Access.SelectProvider('user');
		}
		_OrderPermDisableProvider(providerId);
	}
}