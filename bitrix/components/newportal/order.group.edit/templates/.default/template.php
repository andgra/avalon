...
$arResult['FIELDS'][$arResult['FORMED_GROUP_TAB_ID']][] = array(
	'id' => 'DIRECTION_FORMED_GROUP_ROWS',
	'colspan' => true,
...
if($arResult['ELEMENT']['ID']!=''):
	...
	$arResult['EVENT_EDITOR_ID'] = 'group_'.strval($arParams['ELEMENT_ID']).'_events';
	if($arParams['ELEMENT_ID'] !='') {
		ob_start();
		$APPLICATION->IncludeComponent('newportal:order.event.view',
			'',
			array(
				'AJAX_OPTION_ADDITIONAL' => "GROUP_{$arResult['ELEMENT']['ID']}_EVENT",
				'ENTITY_TYPE' => 'group',
	...
	$arResult['FIELDS'][$arResult['EVENT_TAB_ID']][] = array(
		'id' => 'GROUP_EVENT',
		'name' => GetMessage('ORDER_FIELD_GROUP_EVENT'),
		'colspan' => true,
...
$arResult['ORDER_CUSTOM_PAGE_TITLE'] =
	...
	: GetMessage('ORDER_GROUP_CREATE_TITLE');
/*
*/
$formCustomHtml = '<input type="hidden" name="group_id" value="'.$elementID.'"/>'.$arResult['FORM_CUSTOM_HTML'];
