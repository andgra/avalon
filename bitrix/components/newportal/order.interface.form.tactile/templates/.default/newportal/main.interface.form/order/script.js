if(typeof(BX.OrderFormMode) === "undefined")
{
	BX.OrderFormMode =
	{
		undefined: 0,
...
if(typeof(BX.OrderEditFormManager) === "undefined")
{
	BX.OrderEditFormManager = function()
	{
		this._id = "";
		this._settings = null;
		this._form = null;
		this._formId = "";
		this._tabId = "";
		this._mode = BX.OrderFormMode.edit;
		this._settingsManager = null;
		this._userFieldManager = null;
	};
	BX.OrderEditFormManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._mode = this.getSetting("mode", BX.OrderFormMode.edit);

			this._settingsManager = BX.OrderFormSettingManager.create(
				this._id,
				{
					formId: this._formId,
					manager: this,
			...
			{
				this._userFieldManager = BX.OrderFormUserFieldManager.create(
					this._id,
					{
						manager: this,
	...
	BX.OrderEditFormManager.items = {};
	BX.OrderEditFormManager.create = function(id, settings)
	{
		var self = new BX.OrderEditFormManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
}
if(typeof(BX.OrderFormSettingManager) === "undefined")
{
	BX.OrderFormSettingManager = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.OrderFormSettingManager.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : BX.util.getRandomString(8);
			...
			this._dragContainer = BX.OrderFormSectionDragContainer.create(
				this._id,
				{
					manager: this,
			...
			BX.onCustomEvent(this, "OrderFormSettingManagerCreate", [this]);
		...
		_initializeFromMetaData: function()
		{
			...
			for(var fieldId in fields)
			{
				...
				if(type === "section")
				{
					...
					this._sectionSettings[fieldId] = BX.OrderFormSectionSetting.create(
						fieldId,
				...
				else
				{
					...
					this._fieldSettings[fieldId] = BX.OrderFormFieldSetting.create(
						fieldId,
						{ manager: this, data: info }
					);
				}
		...
		createTemporaryField: function(type, section)
		{
			...
			var field = BX.OrderFormFieldSetting.create(
				temporaryId,
				{
					...
					editMode: true,
					manager: this,
					node: BX.OrderFormFieldRenderer.renderUserFieldRow(userField, table, index)
				}
			);
		...
		createSection: function(precedingSection)
		{
			...
			var table = BX.OrderFormFieldRenderer.renderSectionTable(data, precedingSection.getContentsNode(), this.canCreateSection(), this.canCreateUserField());
			this._sectionSettings[sectionId] = BX.OrderFormSectionSetting.create(
				sectionId,
				{
					data: data,
					editMode: true,
		...
		createDragEffectNode: function(item)
		{
			...
			var label = BX.findChild(item.getNode(), { tagName: "SPAN", className: "order-offer-info-label" }, true, false);
			if(label)
			{
			...
			return BX.create("DIV",
				{
					attrs: { className: "order-offer-draggable-item" },
					children:
					[
						BX.create("SPAN", { attrs: { className: "order-offer-drg-btn" } }),
						BX.create("SPAN", { attrs: { className: "order-offer-title-text" }, html: titleHtml })
					]
				}
			);
		},
		...
		processSectionRemove: function(section)
		{
			...
			BX.findParent(sectionTable, { tagName: "DIV", className: "order-offer-main-wrap" }).removeChild(sectionTable);
		...
		{
			var m = BX.OrderFormSettingManager.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		_findNextDragDropFieldNode: function(node)
		{
			return BX.findNextSibling(node, { tagName: "TR", className: "order-offer-row" });
		},
		_findNextDragDropSectionNode: function(node)
		{
			return BX.findNextSibling(node, { tagName: "TABLE", className: "order-offer-info-table" });
		},
		...
		_onUserFieldCreate: function(sender, temporaryId, userField)
		{
			...
			var node = BX.OrderFormFieldRenderer.renderUserFieldRow(userField, table, (table.rows.length - 1));
			this._fieldSettings[fieldId] = BX.OrderFormFieldSetting.create(fieldId, { data: data, manager: this, node: node });
		...
		{
			if(draggedItem instanceof BX.OrderFormFieldDragItem)
			{
			...
			else if(draggedItem instanceof BX.OrderFormSectionDragItem)
			{
		...
		{
			return BX.OrderFormFieldDragItem.contextId;
		},
		resolveDraggableFieldId: function(contextData)
		{
			...
			if(contextId !== BX.OrderFormFieldDragItem.contextId)
			{
				return "";
			}
			...
			if(oldIndex >= 0 && this.setFieldIndex(fieldId, newIndex, oldIndex))
			{
				...
				if(undoContainer)
				{
					...
					undoContainer.appendChild(
						BX.create("DIV",
							{
								props: { className: "order-view-message" },
								children:
								[
		...
		processDraggedSectionDrop: function(draggedItem)
		{
			...
			if(contextId !== BX.OrderFormSectionDragItem.contextId)
			{
				return;
			}
		...
		createPlaceHolder: function(info)
		{
			...
			var node = BX.create("TABLE", { attrs: { className: "order-offer-info-table" } });
			if(section)
			{
			...
			this._placeHolder = BX.OrderFormSectionPlaceholder.create(
				{
					manager: this,
					node: node,
					sectionId: sectionId,
	...
	if(typeof(BX.OrderFormSettingManager.messages) === "undefined")
	{
		BX.OrderFormSettingManager.messages = {};
	}
	BX.OrderFormSettingManager.items = {};
	BX.OrderFormSettingManager.create = function(id, settings)
	{
		var self = new BX.OrderFormSettingManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.OrderFormFieldSetting) === "undefined")
{
	BX.OrderFormFieldSetting = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.OrderFormFieldSetting.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : BX.util.getRandomString(8);
			...
			if(!(this._manager instanceof BX.OrderFormSettingManager))
			{
				throw "Error: The 'manager' argument must be OrderFormSettingManager instance.";
			}
			...
			var result = cssQuery('span.order-offer-drg-btn', this._node);
			if(result.length > 0)
			{
				this._dragButton = result[0];
				BX.bind(this._dragButton, "contextmenu", this._contextMenuHandler);
			}
			result = cssQuery('span.order-offer-item-edit', this._node);
			if(result.length > 0)
			{
			...
			result = cssQuery('span.order-offer-item-del', this._node);
			if(result.length > 0)
			{
				this._delButton = result[0];
			}

			result = cssQuery('div.order-offer-info-label-wrap', this._node);
			if(result.length > 0)
			{
				this._labelWrapper = result[0];
			}

			result = cssQuery('div.order-offer-info-data-wrap', this._node);
			if(result.length === 0)
			{
		...
		getMessage: function(name)
		{
			var m = BX.OrderFormFieldSetting.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		//D&D abilities
		createGhostNode: function()
		{
			var node = BX.create("DIV", { attrs: { className: "order-offer-draggable-item" } });
			node.appendChild(
				BX.create("SPAN", { attrs: { className: "order-offer-drg-btn" } })
			);
			node.appendChild(
				BX.create("SPAN", { attrs: { className: "order-offer-title-text" }, text: this.getName() })
			);
			return node;
		},
		initializeDragDropAbilities: function()
		{
			...
			if(!this._dragButton)
			{
				throw "OrderFormFieldSetting: Could not find drag button.";
			}

			this._dragItem = BX.OrderFormFieldDragItem.create(
				this.getId(),
				{
					field: this,
		...
		_processBooleanFieldModeChange: function()
		{
			if(this._editMode)
			{
				BX.addClass(this._node, "order-offer-new-item");
				...
				else
				{
					this._nameInput = BX.create(
						"INPUT",
						{
							props: { type: "text", className: "order-offer-item-inp order-offer-label-inp", placeholder: this.getMessage("fieldNamePlaceHolder") }
						}
					);
			...
			{
				BX.removeClass(this._node, "order-offer-new-item");

				this._nameInput.style.display = "none";
				this._buttonWrapper.style.display = "none";
				this._dataWrapper.style.display = "";

				var result = cssQuery('label.order-offer-label', this._dataWrapper);
				if(result.length > 0)
				{
		...
		{
			var result = cssQuery('div.order-offer-address-title', this._node);
			if(result.length === 0)
			{
			...
			result = cssQuery('div.order-offer-addres-title-contents-wrapper', titleContainer);
			if(result.length === 0)
			{
			...
			if(this._editMode)
			{
				...
				else
				{
					this._nameInput = BX.create(
						"INPUT",
						{
							props: { type: "text", className: "order-offer-item-inp", placeholder: this.getMessage("fieldNamePlaceHolder") }
						}
					);
				...
				if(this._manager.getFormMode() === BX.OrderFormMode.edit)
				{
					if(this._cover)
					{
					...
					{
						this._cover = BX.create("DIV", { props: { className: "order-offer-disable-cover" } });
			...
			else
			{
				...
				result = cssQuery('span.order-offer-address-title-contents', titleWrapper);
				if(result.length === 0)
				{
		...
		{
			var result = cssQuery('div.order-offer-editor-title', this._node);
			if(result.length === 0)
			{
				throw "Error: Could not find title container.";
			}
			...
			result = cssQuery('div.order-offer-editor-title-contents-wapper', titleContainer);
			if(result.length === 0)
			{
				throw "Error: Could not find title wrapper.";
			}
			...
			else
			{
				...
				result = cssQuery('span.order-offer-editor-title-contents', titleWrapper);
				if(result.length === 0)
				{
					throw "Error: Could not find title content.";
				}
		...
		_processTextFieldModeChange: function()
		{
			...
			else
			{
				...
				var result = cssQuery('span.order-offer-info-label', this._labelWrapper);
				if(result.length > 0)
				{
					result[0].innerHTML = BX.util.htmlspecialchars(BX.type.isNotEmptyString(this._data["name"]) ? this._data["name"] : this._id);
				}
		...
		_ensureEditButtonsCreated: function()
		{
			...
			this._cancelButton = BX.create(
				"SPAN",
				{
					props: { className: "order-offer-cancel-link" },
			...
			this._buttonWrapper = BX.create(
				"DIV",
				{
					props: { className: "order-offer-item-btn-wrap" },
	...
	if(typeof(BX.OrderFormFieldSetting.messages) === "undefined")
	{
		BX.OrderFormFieldSetting.messages = {};
	}
	BX.OrderFormFieldSetting.create = function(id, settings)
	{
		var self = new BX.OrderFormFieldSetting();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.OrderFormSectionSetting) === "undefined")
{
	BX.OrderFormSectionSetting = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.OrderFormSectionSetting.prototype =
	{
		initialize: function(id, settings)
		{
			...
			var result = cssQuery('div.order-offer-title', BX(idPrefix));
			if(result.length > 0)
			{
			...
			result = cssQuery('span.order-offer-drg-btn', BX(idPrefix));
			if(result.length > 0)
			{
				this._dragButton = result[0];
			...
			result = cssQuery('span.order-offer-title-text', this._titleNode);
			if(result.length === 0)
			{
		...
		getMessage: function(name)
		{
			var m = BX.OrderFormSectionSetting.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		enableEditMode: function(enable, forced)
		{
			...
			if(this._editMode)
			{
				...
				else
				{
					this._titleInput = BX.create(
						"INPUT",
						{
							props: { type: "text", className: "order-item-table-inp", placeholder: this.getMessage("sectionTitlePlaceHolder") }
						}
					);
		...
		initializeDragDropAbilities: function()
		{
			...
			{
				this._dragContainer = BX.OrderFormFieldDragContainer.create(
					this.getId(),
					{
						section: this,
			...
			if(!this._dragItem)
			{
				this._dragItem = BX.OrderFormSectionDragItem.create(
					this.getId(),
					{
						section: this,
						node: this._dragButton,
		...
		createPlaceHolder: function(info)
		{
			...
			this._placeHolder = BX.OrderFormFieldPlaceholder.create(
				{
					section: this,
		...
		_openAddFieldMenu: function()
		{
			...
			if(canCreateUserField)
			{
				menuItems.push(
					{
						id: "addStringField",
						className: "order-offer-popup-item menu-popup-no-icon",
	...
	if(typeof(BX.OrderFormSectionSetting.messages) === "undefined")
	{
		BX.OrderFormSectionSetting.messages = {};
	}
	BX.OrderFormSectionSetting.create = function(id, settings)
	{
		var self = new BX.OrderFormSectionSetting();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.OrderFormUserFieldManager) === "undefined")
{
	BX.OrderFormUserFieldManager = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.OrderFormUserFieldManager.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : BX.util.getRandomString(8);
		...
		createTemporaryField: function(params)
		{
			...
			return BX.OrderFormUserField.create(
				temporaryId,
				{
					manager: this,
					fieldData: fieldData
		...
		_onCreateFieldRequestSuccess: function(data)
		{
			...
			var field = BX.OrderFormUserField.create(fieldName, { manager: this, fieldData: fieldData });
	...
	BX.OrderFormUserFieldManager.items = {};
	BX.OrderFormUserFieldManager.create = function(id, settings)
	{
		var self = new BX.OrderFormUserFieldManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
}
if(typeof(BX.OrderFormUserField) === "undefined")
{
	BX.OrderFormUserField = function()
	{
		this._id = "";
	...
	BX.OrderFormUserField.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : BX.util.getRandomString(8);
			...
			this._elements["value"] = BX.create("INPUT",
				{
					props:
					{
						className: "order-offer-item-inp",
						type: "text",
		...
		_prepareBooleanFieldLayout: function()
		{
			...
			this._elements['checkbox'] = BX.create("INPUT",
				{
					props:
					{
						...
						name: this._fieldName,
						type: "checkbox",
						className: "order-offer-checkbox",
						value: "Y"
					}
				}
			);
			...
			this._elements["label"] =
				BX.create(
					"LABEL",
					{
						props: { className: "order-offer-label" },
						attrs: { "for": chbxId },
						text: this._label
					}
				);
			...
			this._elements["value"] = BX.create("INPUT",
				{
					props:
					{
						className: "order-offer-item-inp order-item-table-date",
						type: "text",
						id: this._fieldName,
						name: this._fieldName,
						value: ""
	...
	BX.OrderFormUserField.create = function(id, settings)
	{
		var self = new BX.OrderFormUserField();
		self.initialize(id, settings);
		return self;
	}
}
if(typeof(BX.OrderFormFieldRenderer) === "undefined")
{
	BX.OrderFormFieldRenderer = function() {};
	BX.OrderFormFieldRenderer.renderUserFieldRow = function(field, table, index)
	{
		...
		row.className = "order-offer-row";

		var cell = row.insertCell(-1);
		cell.className = "order-offer-info-drg-btn";
		cell.appendChild(
			BX.create("SPAN", { props: { className: "order-offer-drg-btn" } })
		);

		cell = row.insertCell(-1);
		cell.className = "order-offer-info-left";
		cell.appendChild(
			BX.create("DIV",
				{
					props: { className: "order-offer-info-label-wrap" },
					children:
					[
						BX.create("SPAN", { props: { className: "order-offer-info-label-alignment" } }),
						BX.create("SPAN",
							{
								props: { className: "order-offer-info-label" },
		...
		cell = row.insertCell(-1);
		cell.className = "order-offer-info-right";
		cell.appendChild(
			BX.create("DIV",
				{
					props: { className: "order-offer-info-data-wrap" },
		...
		cell = row.insertCell(-1);
		cell.className = "order-offer-info-right-btn";
		cell.appendChild(BX.create("SPAN", { props: { className: "order-offer-item-del" } }));
		cell.appendChild(BX.create("SPAN", { props: { className: "order-offer-item-edit" } }));

		cell = row.insertCell(-1);
		cell.className = "order-offer-last-td";

		return row;
	};
	BX.OrderFormFieldRenderer.renderSectionTable = function(data, anchorNode, canCreateSection, canCreateUserField)
	{
		var id = data["id"];
		...
		var table = BX.create(
			"TABLE",
			{ props: { id: prefix + "_contents", className: "order-offer-info-table" } }
		);
		...
		cell.appendChild(
			BX.create(
				"DIV",
				{
					props: { className: "order-offer-title" },
					children:
					[
						BX.create("SPAN", { props: { className: "order-offer-drg-btn" } }),
						BX.create("SPAN", { props: { className: "order-offer-title-text" }, text: name }),
						BX.create(
							"SPAN",
							{
								props: { className: "order-offer-title-set-wrap" },
								text: name,
								children:
								[
									BX.create(
										"SPAN",
										{
											...
											{
												id: prefix + "_edit",
												className: "order-offer-title-edit"
											}
										}
									),
									BX.create(
										"SPAN",
										{
											...
											{
												id: prefix + "_delete",
												className: "order-offer-title-del"
											}
										}
									)
								]
		...
		if (canCreateUserField)
		{
			children.push(BX.create(
				"SPAN",
				{
					props: { id: prefix + "_add_field", className: "order-offer-info-link" },
		...
		if (canCreateSection)
		{
			children.push(BX.create(
				"SPAN",
				{
					props: { id: prefix + "_add_section", className: "order-offer-info-link" },
		...
		children.push(BX.create(
			"SPAN",
			{
				props: { id: prefix + "_restore_field", className: "order-offer-info-link" },
		...
		cell.appendChild(
			BX.create(
				"DIV",
				{
					props: { className: "order-offer-item-link-wrap" },
					children: children
				}
			)
		);
		...
		var targetNode = BX.findNextSibling(anchorNode, { tagName: "TABLE", className: "order-offer-info-table" });
		if(targetNode)
		{
	...
	BX.OrderFormFieldRenderer.getMessage = function(name)
	{
		var m = BX.OrderFormFieldRenderer.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	};
	if(typeof(BX.OrderFormFieldRenderer.messages) === "undefined")
	{
...
//Placeholders
if(typeof(BX.OrderFormFieldPlaceholder) === "undefined")
{
	BX.OrderFormFieldPlaceholder = function()
	{
		this._settings = null;
		this._node = null;
		this._section = null;
	...
	BX.OrderFormFieldPlaceholder.prototype =
	{
		...
		layout: function()
		{
			if(!this._node)
			{
				throw "OrderFormFieldPlaceholder: The 'node' is not assigned.";
			}

			var row = this._node;
			var cell = row.insertCell(-1);
			cell.className = "order-offer-table-cap order-offer-target-place";
			cell.colSpan = 5;
		}
	};
	BX.OrderFormFieldPlaceholder.create = function(settings)
	{
		var self = new BX.OrderFormFieldPlaceholder();
		self.initialize(settings);
		return self;
	};
}
if(typeof(BX.OrderFormSectionPlaceholder) === "undefined")
{
	BX.OrderFormSectionPlaceholder = function()
	{
		this._settings = null;
		this._node = null;
		this._manager = null;
	...
	BX.OrderFormSectionPlaceholder.prototype =
	{
		initialize: function(settings)
		{
			this._settings = settings ? settings : {};
		...
		layout: function()
		{
			if(!this._node)
			{
				throw "OrderFormSectionPlaceholder: The 'node' is not assigned.";
			}
	...
	BX.OrderFormSectionPlaceholder.create = function(settings)
	{
		var self = new BX.OrderFormSectionPlaceholder();
		self.initialize(settings);
		return self;
	};
}
...
//D&D Items
if(typeof(BX.OrderFormFieldDragItem) === "undefined")
{
	BX.OrderFormFieldDragItem = function()
	{
		BX.OrderFormFieldDragItem.superclass.constructor.apply(this);
		this._field = null;
		this._showFieldInDragMode = true;
	};
	BX.extend(BX.OrderFormFieldDragItem, BX.OrderCustomDragItem);
	BX.OrderFormFieldDragItem.prototype.doInitialize = function()
	{
		this._field = this.getSetting("field");
		if(!this._field)
		{
			throw "OrderFormFieldDragItem: The 'field' parameter is not defined in settings or empty.";
		}

		this._showFieldInDragMode = this.getSetting("showFieldInDragMode", true);
	};
	BX.OrderFormFieldDragItem.prototype.getField = function()
	{
		return this._field;
	};
	BX.OrderFormFieldDragItem.prototype.createGhostNode = function()
	{
		if(this._ghostNode)
		{
	...
	BX.OrderFormFieldDragItem.prototype.removeGhostNode = function()
	{
		if(this._ghostNode)
		{
	...
	BX.OrderFormFieldDragItem.prototype.getContextId = function()
	{
		return BX.OrderFormFieldDragItem.contextId;
	};
	BX.OrderFormFieldDragItem.prototype.getContextData = function()
	{
		return ({ contextId: BX.OrderFormFieldDragItem.contextId, field: this._field });
	};
	BX.OrderFormFieldDragItem.prototype.processDragStart = function()
	{
		BX.OrderFormSectionDragContainer.enable(false);
		if(!this._showFieldInDragMode)
		{
			this._field.getNode().style.display = "none";
		}
		BX.OrderFormFieldDragContainer.refresh();
	};
	BX.OrderFormFieldDragItem.prototype.processDragStop = function()
	{
		BX.OrderFormSectionDragContainer.enableAfter(true, 300);
		if(!this._showFieldInDragMode)
		{
			this._field.getNode().style.display = "";
		}
		BX.OrderFormFieldDragContainer.refreshAfter(300);
	};
	BX.OrderFormFieldDragItem.contextId = "form_field_item";
	BX.OrderFormFieldDragItem.create = function(id, settings)
	{
		var self = new BX.OrderFormFieldDragItem();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.OrderFormSectionDragItem) === "undefined")
{
	BX.OrderFormSectionDragItem = function()
	{
		BX.OrderFormSectionDragItem.superclass.constructor.apply(this);
		this._section = null;
		this._showSectionInDragMode = true;
		this._showFieldsInDragMode = true;
	};
	BX.extend(BX.OrderFormSectionDragItem, BX.OrderCustomDragItem);
	BX.OrderFormSectionDragItem.prototype.doInitialize = function()
	{
		this._section = this.getSetting("section");
		if(!this._section)
		{
			throw "OrderFormSectionDragItem: The 'section' parameter is not defined in settings or empty.";
		}
	...
	BX.OrderFormSectionDragItem.prototype.getSection = function()
	{
		return this._section;
	};
	BX.OrderFormSectionDragItem.prototype.createGhostNode = function()
	{
		if(this._ghostNode)
		{
			return this._ghostNode;
	...
	BX.OrderFormSectionDragItem.prototype.removeGhostNode = function()
	{
		if(this._ghostNode)
		{
			document.body.removeChild(this._ghostNode);
	...
	BX.OrderFormSectionDragItem.prototype.getContextId = function()
	{
		return BX.OrderFormSectionDragItem.contextId;
	};
	BX.OrderFormSectionDragItem.prototype.getContextData = function()
	{
		return ({ contextId: BX.OrderFormSectionDragItem.contextId, section: this._section });
	};
	BX.OrderFormSectionDragItem.prototype.processDragStart = function()
	{
		BX.OrderFormFieldDragContainer.enable(false);
		...
		BX.OrderFormSectionDragContainer.refresh();
	};
	BX.OrderFormSectionDragItem.prototype.processDragStop = function()
	{
		BX.OrderFormFieldDragContainer.enableAfter(true, 300);
		...
		BX.OrderFormSectionDragContainer.refreshAfter(300);
	};
	BX.OrderFormSectionDragItem.contextId = "form_section_item";
	BX.OrderFormSectionDragItem.create = function(id, settings)
	{
		var self = new BX.OrderFormSectionDragItem();
		self.initialize(id, settings);
		return self;
	};
}
...
//D&D Containers
if(typeof(BX.OrderFormFieldDragContainer) === "undefined")
{
	BX.OrderFormFieldDragContainer = function()
	{
		BX.OrderFormFieldDragContainer.superclass.constructor.apply(this);
		this._section = null;
	};
	BX.extend(BX.OrderFormFieldDragContainer, BX.OrderCustomDragContainer);
	BX.OrderFormFieldDragContainer.prototype.doInitialize = function()
	{
		this._section = this.getSetting("section");
		if(!this._section)
		{
			throw "OrderFormFieldDragContainer: The 'section' parameter is not defined in settings or empty.";
		}
	};
	BX.OrderFormFieldDragContainer.prototype.getSection = function()
	{
		return this._section;
	};
	BX.OrderFormFieldDragContainer.prototype.createPlaceHolder = function(pos)
	{
		var rect;
	...
	BX.OrderFormFieldDragContainer.prototype.removePlaceHolder = function()
	{
		this._section.removePlaceHolder();
	};
	BX.OrderFormFieldDragContainer.prototype.isAllowedContext = function(contextId)
	{
		return (contextId === BX.OrderFormFieldDragItem.contextId);
	};
	BX.OrderFormFieldDragContainer.enable = function(enable)
	{
	...
	BX.OrderFormFieldDragContainer.enableAfter = function(enable, interval)
	{
		interval = parseInt(interval);
		if(interval > 0)
		{
			window.setTimeout(function() { BX.OrderFormFieldDragContainer.enable(enable); });
		}
		else
		{
			this.enable(enable);
	...
	BX.OrderFormFieldDragContainer.refresh = function()
	{
		for(var k in this.items)
		{
			if(this.items.hasOwnProperty(k))
	...
	BX.OrderFormFieldDragContainer.refreshAfter = function(interval)
	{
		interval = parseInt(interval);
		if(interval > 0)
		{
			window.setTimeout(function() { BX.OrderFormFieldDragContainer.refresh(); }, interval);
		}
		else
		{
			this.refresh();
	...
	BX.OrderFormFieldDragContainer.items = {};
	BX.OrderFormFieldDragContainer.create = function(id, settings)
	{
		var self = new BX.OrderFormFieldDragContainer();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
}
if(typeof(BX.OrderFormSectionDragContainer) === "undefined")
{
	BX.OrderFormSectionDragContainer = function()
	{
		BX.OrderFormSectionDragContainer.superclass.constructor.apply(this);
		this._manager = null;
	};
	BX.extend(BX.OrderFormSectionDragContainer, BX.OrderCustomDragContainer);
	BX.OrderFormSectionDragContainer.prototype.doInitialize = function()
	{
		this._manager = this.getSetting("manager");
		if(!this._manager)
		{
			throw "OrderFormSectionDragContainer: The 'manager' parameter is not defined in settings or empty.";
		}
	};
	BX.OrderFormSectionDragContainer.prototype.getManager = function()
	{
		return this._manager;
	};
	BX.OrderFormSectionDragContainer.prototype.createPlaceHolder = function(pos)
	{
		var rect;
	...
	BX.OrderFormSectionDragContainer.prototype.removePlaceHolder = function()
	{
		this._manager.removePlaceHolder();
	};
	BX.OrderFormSectionDragContainer.prototype.isAllowedContext = function(contextId)
	{
		return (contextId === BX.OrderFormSectionDragItem.contextId);
	};
	BX.OrderFormSectionDragContainer.enable = function(enable)
	{
		for(var k in this.items)
		{
			if(this.items.hasOwnProperty(k))
	...
	BX.OrderFormSectionDragContainer.enableAfter = function(enable, interval)
	{
		interval = parseInt(interval);
		if(interval > 0)
		{
			window.setTimeout(function() { BX.OrderFormSectionDragContainer.enable(enable); });
		}
		else
		{
			this.enable(enable);
	...
	BX.OrderFormSectionDragContainer.refresh = function()
	{
		for(var k in this.items)
		{
			if(this.items.hasOwnProperty(k))
	...
	BX.OrderFormSectionDragContainer.refreshAfter = function(interval)
	{
		interval = parseInt(interval);
		if(interval > 0)
		{
			window.setTimeout(function() { BX.OrderFormSectionDragContainer.refresh(); }, interval);
		}
		else
		{
			this.refresh();
	...
	BX.OrderFormSectionDragContainer.items = {};
	BX.OrderFormSectionDragContainer.create = function(id, settings)
	{
		var self = new BX.OrderFormSectionDragContainer();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
