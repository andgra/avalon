...
function OrderPermRoleShowRow(el)
{
	el.className = (el.className == 'orderPermRoleTreePlus' ? 'orderPermRoleTreeMinus' : 'orderPermRoleTreePlus');
	var parentTr = BX.findParent(el, {tag:'tr'});

	while(parentTr.nextElementSibling.nodeName == 'TR')
	{
		if (parentTr.nextElementSibling.className == 'orderPermRoleFields')
		{
...
function OrderPermRoleShowBox(id, parent_id)
{
