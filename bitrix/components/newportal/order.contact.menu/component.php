...
if ($COrderPerms->HavePerm('CONTACT', BX_ORDER_PERM_NONE))
	return;
...
{
	$bRead   = !$COrderPerms->HavePerm('CONTACT', BX_ORDER_PERM_NONE, 'READ');
	$bAdd    = !$COrderPerms->HavePerm('CONTACT', BX_ORDER_PERM_NONE, 'ADD');
	$bEdit  = !$COrderPerms->HavePerm('CONTACT', BX_ORDER_PERM_NONE, 'EDIT');
	//$bAdd =false;
	$bDelete = false;
}
else
{
	$bRead   = COrderContact::CheckReadPermission($arParams['ELEMENT_ID'], $COrderPerms);
	$bAdd    = COrderContact::CheckCreatePermission($COrderPerms);
	$bEdit  = COrderContact::CheckUpdatePermission($arParams['ELEMENT_ID'], $COrderPerms);
	$bDelete = COrderContact::CheckDeletePermission($arParams['ELEMENT_ID'], $COrderPerms);
}
...
if($arParams['TYPE'] === 'list')
{
	...
	{
		$arResult['BUTTONS'][] = array(
			'TEXT' => GetMessage('CONTACT_ADD'),
			'TITLE' => GetMessage('CONTACT_ADD_TITLE'),
...
{
	$arResult['BUTTONS'][] = array(
		'TEXT' => GetMessage('CONTACT_DELETE'),
		'TITLE' => GetMessage('CONTACT_DELETE_TITLE'),
		'LINK' => "javascript:contact_delete('".GetMessage('CONTACT_DELETE_DLG_TITLE')."', '".GetMessage('CONTACT_DELETE_DLG_MESSAGE')."', '".GetMessage('CONTACT_DELETE_DLG_BTNTITLE')."', '".CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONTACT_EDIT'],
			array(
