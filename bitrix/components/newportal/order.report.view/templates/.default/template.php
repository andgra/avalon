...
<div id="report-chfilter-examples-custom" style="display: none;">
	...
	<div class="filter-field filter-field-type chfilter-field-ASSIGNED_TYPE" callback="RTFilter_chooseBoolean">
		...
		<select id="%ID%" name="%NAME%" class="filter-dropdown" caller="true">
			...
			<?
				$arTypes = COrderHelper::GetEnumList('ACCESS','PROVIDER');
			?>
	...
	<div class="filter-field filter-field-type chfilter-field-ROOT_ID" callback="RTFilter_chooseBoolean">
		<label for="%ID%" class="filter-field-title">%TITLE% "%COMPARE%"</label>
		<select id="%ID%" name="%NAME%" class="filter-dropdown" caller="true">
			<option value=""><?=GetMessage('CRM_REPORT_INCLUDE_ALL')?></option>
			<?
...
<?/* //Убрать </div> при снятии комментариев
