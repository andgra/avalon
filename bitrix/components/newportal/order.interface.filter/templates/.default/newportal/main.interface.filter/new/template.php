...
$options = CUserOptions::GetOption('order.interface.grid.filter', strtolower($filterID));
if(!$options)
{
	$options = array(
		'rows' => '',
...
if(!function_exists('__OrderInterfaceFilterRenderField'))
{
	function __OrderInterfaceFilterRenderField(&$field, &$values, &$infos, $options = array())
	{
		if(!is_array($options))
		{
			$options = array();
			...
			case 'list':
				{
					...
					else
					{
						...
						echo '<option value=""',
							$isSelected ? ' selected="selected"' : '', '>',
							htmlspecialcharsbx(GetMessage("ORDER_INTERFACE_FILTER_LIST_VALUE_NOT_SELECTED")), '</option>';
...
?><form name="<?=htmlspecialcharsbx($formName)?>" action="" method="GET">
	<div class="order-main-wrap"<?=$isHidden ? ' style="display:none;"' : ''?>>
		<div id="<?=htmlspecialcharsbx($containerID)?>" class="order-filter-wrap">
						...
						<td class="bx-filter-main-table-cell">
							<div class="bx-filter-tabs-block" id="filter-tabs"><?
								$isActive = !$isFilterFolded
									...
									: ($isFilterApplied && $currentFilterID === '');
								?><span id="<?=htmlspecialcharsbx("{$tabPrefix}filter_default")?>" class="bx-filter-tab<?=$isActive ? ' bx-filter-tab-active' : ''?><?=$isFilterApplied && $currentFilterID === '' ? ' bx-current-filter-tab' : ''?>"><?= GetMessage('ORDER_INTERFACE_FILTER_CURRENT') ?></span><?
								...
								endforeach;
								unset($item);
								?><span class="bx-filter-tab bx-filter-add-tab" title="<?=htmlspecialcharsbx(GetMessage('ORDER_INTERFACE_FILTER_ADD'))?>"></span>
						...
						<td class="bx-filter-main-table-cell">
							<div class="bx-filter-content<?=$visibileFieldCount > 1 ? '' : ' bx-filter-content-first'?>">
											...
											<td class="bx-filter-item-center">
												<div class="bx-filter-alignment">
													<div class=" bx-filter-box-sizing"><?
														__OrderInterfaceFilterRenderField(
															$field,
															$values,
															$infos,
															array(
								...
								<div class="bx-filter-bottom">
									<input value="<?=htmlspecialcharsbx(GetMessage('ORDER_INTERFACE_FILTER_FIND'))?>" name="set_filter" type="button"/>
									<input value="<?=htmlspecialcharsbx(GetMessage('ORDER_INTERFACE_FILTER_CANCEL'))?>" name="reset_filter" type="button"/>
									...
									<div class="bx-filter-setting-block">
										<span class="bx-filter-setting" title="<?=htmlspecialcharsbx(GetMessage('ORDER_INTERFACE_FILTER_SETTINGS'))?>"></span>
										<span class="bx-filter-add-button" title="<?=htmlspecialcharsbx(GetMessage('ORDER_INTERFACE_FILTER_ADD_FIELD'))?>"></span>
									</div>
								</div>
							<div>
						</td>
	...
	BX.ready(
			function()
			{
				BX.InterfaceGridFilter.messages =
					{
						"showAll": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SHOW_ALL")?>",
						"hideAll": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_HIDE_ALL")?>",
						"saveAs": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE_AS")?>",
						"save": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE")?>",
						"delete": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_DELETE")?>",
						"saveAsDialogTitle": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE_AS_DIALOG_TITLE")?>",
						"saveAsDialogFieldName": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE_AS_DIALOG_FIELD_NAME")?>",
						"defaultFilterName": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE_AS_DIALOG_FIELD_NAME_DEFAULT")?>",
						"buttonSave": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_SAVE")?>",
						"buttonCancel": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_CANCEL")?>",
						"buttonMinimize": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_MINIMIZE")?>",
						"buttonMaximize": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_MAXIMIZE")?>",
						"buttonDeleteField": "<?=GetMessageJS("ORDER_INTERFACE_FILTER_DELETE_FIELD")?>"
					};
