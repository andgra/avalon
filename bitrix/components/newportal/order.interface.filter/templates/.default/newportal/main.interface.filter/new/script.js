if(typeof(BX.InterfaceGridFilter) === "undefined")
{
	...
	BX.InterfaceGridFilter.prototype =
	{
		initialize: function(id, settings)
		{
			...
			for(var fieldId in this._fieldInfos)
			{
				...
				var f =
					BX.InterfaceGridFilterField.create(
						fieldId,
						BX.OrderParamBag.create({ "filter": this, "info": this._fieldInfos[fieldId] })
					);
			...
			this._manager = BX.OrderInterfaceGridManager.items[this.getGridId() + '_MANAGER'];
			if(this._manager)
			{
			...
			else
			{
				...
				this._manager = null;
				BX.addCustomEvent(BX.OrderInterfaceGridManager, 'CREATED', this._managerHandler);
			}
		},
		release: function()
		{
			...
			{
				BX.removeCustomEvent(BX.OrderInterfaceGridManager, 'CREATED', this._managerHandler);
			}
		},
		...
		deleteActiveItem: function()
		{
			...
			BX.userOptions.save("order.interface.grid.filter", this.getId().toLowerCase(), "presetsDeleted", this._presetsDeleted.join(','));
		...
		handleSaveAsDialogClose: function(dialog)
		{
			...
			var item = this._items[itemId] = BX.InterfaceGridFilterItem.create(
				itemId,
				BX.OrderParamBag.create({ "filter": this, "info": itemInfo, "isActive": false })
			);
		...
		{
			BX.onCustomEvent(document, 'bx-ui-order-filter-set-active-item', [this, item]);
		...
		setFolded: function(folded)
		{
			...
			var d = BX.userOptions.delay;
			BX.userOptions.delay = 100;
			BX.userOptions.save("order.interface.grid.filter", this.getId().toLowerCase(), "isFolded", (folded ? "Y" : "N"));
			BX.userOptions.delay = d;
		},
	...
	BX.InterfaceGridFilterCheckListMenuItem.prototype =
	{
			...
			result.push(
				{
					...
					"className" : "order-filter-popup-item" + (this.isChecked() ? " menu-popup-item-checked" : "") + " order-check-list-menu-item-" + this.getId().toLowerCase(),
					"href" : "#",
		...
		_onClick: function(e)
		{
			...
			var node = BX.findChild(
				...
				{ "tag":"A", "class": "order-check-list-menu-item-" + this.getId().toLowerCase() },
				true,
				false
			);
