...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_GROUP_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_GROUP_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_GROUP_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['group_id'];
    ...
    $arResult['PATH_TO_GROUP_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_GROUP_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[group_id]=#group_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'GROUP';
