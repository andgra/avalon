...
if(!empty($arParams["FILTER"])):
	...
	"newportal:order.interface.filter",
...
<?=bitrix_sessid_post();?>
<?endif?>
<div class="bx-order-interface-list">
<table cellspacing="0" class="bx-order-interface-grid<?if($arResult["OPTIONS"]["theme"] <> '') echo " bx-order-interface-grid-theme-".$arResult["OPTIONS"]["theme"]?>" id="<?=$arParams["GRID_ID"]?>">
	<thead>
...
<?foreach($arResult["HEADERS"] as $header):?>
		<td<?=($header["sort_state"] <> ''? ' class="bx-order-sorted"':'')?>><div class="empty"></div></td>
<?endforeach?>
	</tr>
	<tr class="bx-order bx-order-grid-head" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu"<?if($GLOBALS['USER']->IsAuthorized()):?> ondblclick="bxGrid_<?=$arParams["GRID_ID"]?>.EditCurrentView()"<?endif?>>
<?if($arResult["ALLOW_EDIT"]):?>
		<td class="bx-order-checkbox-col" width="1%"><input type="checkbox" name="" id="<?=$arParams["GRID_ID"]?>_check_all" value="" title="<?echo GetMessage("interface_grid_check_all")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.SelectAllRows(this);"></td>
<?endif?>
		<td class="bx-order-actions-col" width="1%"><a href="javascript:void(0);"
			...
			title="<?echo GetMessage("interface_grid_settings")?>" class="bx-order-action"><div class="empty"></div></a></td>
<?
...
?>
	<tr class="bx-order-table-body" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>"<?endif?>>
<?if($arResult["ALLOW_EDIT"]):?>
	<?
	if($aRow["editable"] !== false):
		$data_id = ($aRow["id"] <> ''? $aRow["id"] : $aRow["data"]["ID"]);
	?>
		<td class="bx-order-checkbox-col"><input type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?echo GetMessage("interface_grid_check")?>"></td>
	<?else:?>
		<td class="bx-order-checkbox-col">&nbsp;</td>
	<?endif?>
<?endif?>
	<?if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-order-actions-col"><a href="javascript:void(0);"
			onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ShowActionMenu(this, <?=$index?>);"
			title="<?echo GetMessage("interface_grid_act")?>" class="bx-order-action"><div class="empty"></div></a></td>
	<?else:?>
		<td>&nbsp;</td>
	<?endif?>
<?
	...
	foreach($arResult["HEADERS"] as $id=>$header):
	$columnClass = 	$header["sort_state"] !== '' ? 'bx-order-sorted' : '';
...
?>
	<tr><td class="bx-order-not-result" colspan="<?=$colspan?>"><?echo GetMessage("interface_grid_no_data")?></td></tr>
<?endif?>
	</tbody>
<?if($arResult["ALLOW_EDIT"] || is_array($arParams["FOOTER"]) && count($arParams["FOOTER"]) > 0 || $arResult["NAV_STRING"] <> ''):?>
	<tfoot>
	<tr class="bx-order-grid-footer">
		<td colspan="<?=$colspan?>">
			<table cellpadding="0" cellspacing="0" border="0" class="bx-order-table-footer bx-order-grid-footer">
				<tbody><tr>
			<?if($arResult["ALLOW_EDIT"]):?>
			...
			<?endforeach?>
					...
					<td class="bx-order-right"><?=($arResult["NAV_STRING"] <> ''? $arResult["NAV_STRING"] : '&nbsp;')?></td>
				</tr></tbody>
			</table>
		</td>
	</tr>
...
<?if($arResult["ALLOW_EDIT"]):?>
<div class="bx-order-footer-interface-toolbar-container">
<input type="hidden" name="action_button_<?=$arParams["GRID_ID"]?>" value="">
<table cellpadding="0" cellspacing="0" border="0" class="">
<!--	<tr class="bx-top"><td class="bx-order-left"><div class="empty"></div></td><td><div class="empty"></div></td><td class="bx-order-right"><div class="empty"></div></td></tr>-->
	<tr>
<!--		<td class="bx-order-left"><div class="empty"></div></td>-->
<!--		<td class="bx-content">-->
...
<?if($arResult["ALLOW_INLINE_EDIT"]):?>
	<?if($bNeedSep):?>
<!--		<td><div class="bx-order-separator"></div></td>-->
	<?endif;?>
		<td class="vam"><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionEdit(this);" title="<?echo GetMessage("interface_grid_edit_selected")?>" class="bx-order-bd-icon-edit" id="edit_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?
	$bNeedSep = true;
endif;
?>
...
<?endif?>
<?if($bShowApply):?>
		<td class="vam" style="padding-left:2px;"><input class="bx-order-btn bx-order-btn-medium bx-order-btn-gray mb0" type="submit" name="apply" value="<?echo GetMessage("interface_grid_apply")?>" disabled></td>
<?endif?>
<!--				</tr>-->
<!--			</table>-->
<!--		</td>-->
		<td class="bx-order-right"><div class="empty"></div></td>
	</tr>
	<tr class="bx-order-bottom"><td class="bx-order-left"><div class="empty"></div></td><td><div class="empty"></div></td><td class="bx-order-right"><div class="empty"></div></td></tr>
</table>
</div>
</div>
<?endif?>
...
<script type="text/javascript">
	bxGrid_<?=$arParams["GRID_ID"]?>.pageSizeControl = new BX.Order.GridPageSizeControl(
		{
...
?><script type="text/javascript">
	BX.ready(
		function()
		{
			...
			var manager = BX.OrderInterfaceGridManager.items[managerId];
			if(manager)
			{
			...
			{
				BX.OrderInterfaceGridManager.create(managerId, <?=CUtil::PhpToJSObject($managerConfig) ?>);
			}
		}
	);
</script>
