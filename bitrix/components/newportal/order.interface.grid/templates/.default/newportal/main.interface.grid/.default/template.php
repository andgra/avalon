...
<?=bitrix_sessid_post();?>
<?endif?>
<div class="bx-order-interface-grid">
...
?>
<?if($arParams["ACTIONS"]["add"] == true):?>
	<?if($bNeedSep && !$arResult["ALLOW_INLINE_EDIT"]):?>
		<td><div class="bx-separator"></div></td>
	<?endif?>
		<td><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionAdd();" title="<?echo GetMessage("interface_grid_add_title")?>" class="context-button icon action-add-button" id="add_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?
	$bNeedSep = true;
endif;
?>
