function BxInterfaceGrid(table_id)
{
				...
				if(el.checked)
				{
					...
					for(var col_id in this.oColsMeta)
					{
						if(this.oColsMeta[col_id].editable == true && this.oEditData[row_id][col_id] !== false)
						{
							...
							{
								case 'structure_selector':
									//console.log(data);
									if(typeof(data)=='object' && data['html']!='undefined') {
										td.appendChild(BX.create('SPAN', {'html': data['html'].replace(new RegExp("%ID%",'g'),row_id)}));
										if(data['id']) {
											BX.ready(
												function () {
													window['obj_' + data['id'].replace(new RegExp("%ID%",'g'),row_id)] = BX.OrderStructure.Set(
														data['id'].replace(new RegExp("%ID%",'g'),row_id),
														data['contId'].replace(new RegExp("%ID%",'g'),row_id),
														data['selected']
													);
												}
											);
										...
										td.appendChild(BX.create('SPAN', {'html': data}));
									}
									break;
									...
									if(typeof(data)=='object' && data['html']!='undefined') {
										...
										if(data['id']) {
											BX.ready(
												function()
												{
													window['obj_' + data['id'].replace(new RegExp("%ID%",'g'),row_id)]=BX.OrderPerson.Set(
														data['id'].replace(new RegExp("%ID%",'g'),row_id),
														data['contId'].replace(new RegExp("%ID%",'g'),row_id),
														data['type'].replace(new RegExp("%ID%",'g'),row_id),
														data['selected']
													);
												}
											);
								...
								case 'custom':
									if(typeof(data)=='object') {
										if(data['ADD_ONLY'] && data['ADD_ONLY']=='Y') {
											break;
										}
										if(data['html']) td.appendChild(BX.create('SPAN', {'html': data['html']}));
										if(data['script']) eval(data['script']);
									} else {
										td.appendChild(BX.create('SPAN', {'html': data}));
									}
								...
								case 'textarea':
									var props = {
										'rows':5,
										'name':name,
										'cols':(this.oColsMeta[col_id].size? this.oColsMeta[col_id].size : 30)
									};
									td.appendChild(BX.create('TEXTAREA', {'props': props,'text':data}));
									break;
								case 'checkbox':
	...
	this.NewRowDelete = function(a)
	{
		...
		if(table.rows.length<=3) {
			table.rows[0].parentNode.
				insertBefore(emptyDataTr, table.rows[table.rows.length-1]);
		}
	};
	...
	this.ActionAdd = function()
	{
		...
		var lastTr=BX(this.table_id).rows[BX(this.table_id).rows.length-2];
		if(BX.hasClass(lastTr.cells[0],'bx-right') && BX.hasClass(lastTr.cells[0],'bx-left')) {
			emptyDataTr=lastTr;
			lastTr.parentNode.removeChild(lastTr);
		}
		var newRow = BX(this.table_id).insertRow(BX(this.table_id).rows.length-1);
		var iCell=0;
		var td;
		if(window['row_new_id'])
			window['row_new_id']++;
		else
			window['row_new_id']=1;

		var tdLeft = newRow.insertCell(iCell++);
		tdLeft.className='bx-left';
		tdLeft.colSpan='2';
		tdLeft.style.textAlign='center';
		tdLeft.appendChild(BX.create('a', {'attrs':{
			'href':"javascript:void(0);",
			'onclick':'bxGrid_'+this.table_id+'.NewRowDelete(this);',
			'className':'bx-order-btn-delete',
			'id':'delete_button_'+this.table_id+'_new_row_'+row_new_id
		}}));
		tdLeft.appendChild(BX.create('input', {'attrs':{
			'type':"hidden",
			'name':'NEW_'+row_new_id+'_FLAG',
			'value':'NEW'
		}}));
		/*var tdActions = newRow.insertCell(iCell++);
		tdActions.className='bx-actions-col';*/

		for(var col_id in this.oColsMeta)
		{
			td = newRow.insertCell(iCell++);
			var meta=this.oColsMeta[col_id];
			if(meta.editable == true)
			{
				//insert controls
				var data = meta['default_value'];

				var name = 'FIELDS[NEW_'+row_new_id+']['+col_id+']';
				switch(this.oColsMeta[col_id].type)
				{
					case 'structure_selector':
						if(data==null) data='';
						if(typeof(data)=='object' && data['html']!='undefined') {
							td.appendChild(BX.create('SPAN', {'html': data['html'].replace(new RegExp("%ID%",'g'),row_new_id)}));
							if(data['id']) {
								BX.ready(
									function () {
										window['obj_' + data['id'].replace(new RegExp("%ID%",'g'),row_new_id)] = BX.OrderStructure.Set(
											data['id'].replace(new RegExp("%ID%",'g'),row_new_id),
											data['contId'].replace(new RegExp("%ID%",'g'),row_new_id),
											data['selected']
										);
									}
								);
						...
						if(typeof(data)=='object' && data['html']!='undefined') {
							...
							if(data['id']) {
								BX.ready(
									function()
									{
										window['obj_' + data['id'].replace(new RegExp("%ID%",'g'),row_new_id)]=BX.OrderPerson.Set(
											data['id'].replace(new RegExp("%ID%",'g'),row_new_id),
											data['contId'].replace(new RegExp("%ID%",'g'),row_new_id),
											data['type'].replace(new RegExp("%ID%",'g'),row_new_id),
											data['selected']
										);
									}
								);
					...
					case 'custom':
						...
						if(typeof(data)=='object') {
							...
							if(data['html']) td.appendChild(BX.create('SPAN', {'html': data['html'].replace(new RegExp("%ID%",'g'),row_new_id)}));
							if(data['script']) eval(data['script'].replace(new RegExp("%ID%",'g'),row_new_id));
						} else {
							td.appendChild(BX.create('SPAN', {'html': data}));
						}
						break;
