...
if ($COrderPerms->HavePerm('GROUP', BX_ORDER_PERM_NONE))
	return;
...
$toolbarID = 'toolbar_group_'.$arParams['TYPE'];
...
{
	$bRead   = !$COrderPerms->HavePerm('GROUP', BX_ORDER_PERM_NONE, 'READ');
	//$bAdd    = !$COrderPerms->HavePerm('GROUP', BX_ORDER_PERM_NONE, 'ADD');
	$bEdit  = !$COrderPerms->HavePerm('GROUP', BX_ORDER_PERM_NONE, 'EDIT');
	$bAdd =false;
	$bDelete = false;
}
else
{
	$bRead   = COrderGroup::CheckReadPermission($arParams['ELEMENT_ID'], $COrderPerms);
	//$bAdd    = COrderGroup::CheckCreatePermission($COrderPerms);
	//$bEdit  = COrderGroup::CheckUpdatePermission($arParams['ELEMENT_ID'], $COrderPerms);
	//$bDelete = COrderGroup::CheckDeletePermission($arParams['ELEMENT_ID'], $COrderPerms);
	$bAdd=false;
	$bEdit=false;
	$bDelete=false;
}
...
if($arParams['TYPE'] === 'list')
{
	if ($bAdd)
	{
		$arResult['BUTTONS'][] = array(
			...
			'LINK' => CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_GROUP_EDIT'],
				array(
					'group_id' => 0
				)
			),
...
{
	$arResult['BUTTONS'][] = array(
		'TEXT' => GetMessage('GROUP_DELETE'),
		'TITLE' => GetMessage('GROUP_DELETE_TITLE'),
		'LINK' => "javascript:group_delete('".GetMessage('GROUP_DELETE_DLG_TITLE')."', '".GetMessage('GROUP_DELETE_DLG_MESSAGE')."', '".GetMessage('GROUP_DELETE_DLG_BTNTITLE')."', '".CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_GROUP_EDIT'],
			array(
				'group_id' => $arParams['ELEMENT_ID']
			)),
