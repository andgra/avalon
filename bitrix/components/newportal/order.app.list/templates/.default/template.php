...
?>
    <script type="text/javascript">
        function order_app_delete_grid(title, message, btnTitle, path)
        {
            var d;
            d = new BX.CDialog({
                title: title,
...
$arData=array();
foreach($arResult['APP'] as $id => $el) {
    $data=array(
        'id' => $id,
        'actions' => array(
            array(
                'ICONCLASS' => 'edit',
                'TITLE' => GetMessage('ORDER_APP_EDIT_TITLE'),
                'TEXT' => GetMessage('ORDER_APP_EDIT'),
                'ONCLICK' => 'jsUtils.Redirect([], \''.$el['PATH_TO_APP_EDIT'].'\');',
                'DEFAULT' => true,
            )
        ),
        'data' => $el,
        'editable' => $el['PERM_EDIT']
    );
    if($el['PERM_DELETE'])
        $data['actions']=array_merge($data['actions'],array(
            array(
                'ICONCLASS' => 'delete',
                'TITLE' => GetMessage('ORDER_APP_DELETE_TITLE'),
                'TEXT' => GetMessage('ORDER_APP_DELETE'),
                'ONCLICK' => "order_app_delete_grid('".CUtil::JSEscape(GetMessage('ORDER_APP_DELETE_TITLE'))."', '".CUtil::JSEscape(GetMessage('ORDER_APP_DELETE_CONFIRM'))."', '".CUtil::JSEscape(GetMessage('ORDER_APP_DELETE'))."', '".CUtil::JSEscape($el['PATH_TO_APP_DELETE'])."')"
            )
        ));
    $row=array(
        'ID' => '<a href="'.$el['PATH_TO_APP_EDIT'].'">'.$el['ID'].'</a>',
        'AGENT_ID' => ($el['AGENT_ID']!=''?'<a href="'.$el['PATH_TO_AGENT_EDIT'].'" target="_blank">'.$el['AGENT_TITLE'].'</a>':$el['AGENT_TITLE'])/*.
            ($el['AGENT_PHONE']!=''?'<br><small style="color:silver">'.$el['AGENT_PHONE'].'</small>':'').
            ($el['AGENT_EMAIL']!=''?'<br><small style="color:silver">'.$el['AGENT_EMAIL'].'</small>':'')*/,
        /*'CONTACT_ID' => ($el['CONTACT_ID']!=''?'<a href="'.$el['PATH_TO_CONTACT_EDIT'].'" target="_blank">'.$el['CONTACT_FULL_NAME'].'</a>':$el['CONTACT_FULL_NAME']).
            ($el['CONTACT_PHONE']!=''?'<br><small style="color:silver">'.$el['CONTACT_PHONE'].'</small>':'').
            ($el['CONTACT_EMAIL']!=''?'<br><small style="color:silver">'.$el['CONTACT_EMAIL'].'</small>':''),*/
...
if($arResult['PERMS']['EDIT'])
{
    ...
    $arActionList['set_status'] =GetMessage('ORDER_APP_SET_STATUS');
    $arActionList['set_assigned'] = GetMessage('ORDER_APP_SET_ASSIGNED');
    $arActionList['set_description'] = GetMessage('ORDER_APP_SET_DESCRIPTION');
