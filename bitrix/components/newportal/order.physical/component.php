...
$componentPage = '';
$arComponentVariables = array('physical_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_PHYSICAL_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_PHYSICAL_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_PHYSICAL_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['physical_id'];
    ...
    $arResult['PATH_TO_PHYSICAL_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_PHYSICAL_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[physical_id]=#physical_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'PHYSICAL';
