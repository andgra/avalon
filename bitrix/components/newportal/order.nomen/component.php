...
$componentPage = '';
$arComponentVariables = array('nomen_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_NOMEN_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_NOMEN_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_NOMEN_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['nomen_id'];
    ...
    $arResult['PATH_TO_NOMEN_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_NOMEN_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[nomen_id]=#nomen_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'NOMEN';
