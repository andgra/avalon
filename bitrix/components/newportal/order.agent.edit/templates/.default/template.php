...
if($arResult['ELEMENT']['ID']!=''):
	...
	$arResult['EVENT_EDITOR_ID'] = 'agent_'.strval($arParams['ELEMENT_ID']).'_events';
	if($arParams['ELEMENT_ID'] !='') {
		ob_start();
		$APPLICATION->IncludeComponent('newportal:order.event.view',
			'',
			array(
				'AJAX_OPTION_ADDITIONAL' => "AGENT_{$arResult['ELEMENT']['ID']}_EVENT",
				'ENTITY_TYPE' => 'agent',
	...
	$arResult['FIELDS'][$arResult['EVENT_TAB_ID']][] = array(
		'id' => 'AGENT_EVENT',
		'name' => GetMessage('ORDER_FIELD_AGENT_EVENT'),
		'colspan' => true,
...
$arResult['ORDER_CUSTOM_PAGE_TITLE'] =
	$elementID !=''
	? GetMessage('ORDER_AGENT_EDIT_TITLE',
		array(
			'#ID#' => $elementID,
			'#TITLE#' => isset($arResult['ELEMENT']['TITLE']) ? $arResult['ELEMENT']['TITLE'] : ''
		)
	)
	: GetMessage('ORDER_AGENT_CREATE_TITLE');
/*
*/
$formCustomHtml = '<input type="hidden" name="agent_id" value="'.$elementID.'"/>'.$arResult['FORM_CUSTOM_HTML'];
...
<script type="text/javascript">
	...
	BX.ready(
		function()
		{
			...
			var select = BX.findChild(BX('legal_wrap'),{'name':'LEGAL','tag':'select'},true);
			BX.bind(select, 'change', function(e) {
				document.location.href = "?legal="+e.target.value;
			});
		}
	);
</script>
