...
$componentPage = '';
$arComponentVariables = array('reg_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_REG_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_REG_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_REG_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['reg_id'];
    ...
    $arResult['PATH_TO_REG_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_REG_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[reg_id]=#reg_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'REG';
