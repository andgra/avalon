...
if(!$bInternal || strtoupper($arResult['EXTERNAL_TYPE'])!='DIRECTION') {
    $arResult['HEADERS'][]=array('id' => 'PARENT_ID', 'name' => GetMessage('ORDER_COLUMN_PARENT'), 'default' => true, 'sort' => 'PARENT_TITLE', 'editable' => false);
}
...
if(!$bInternal) {
    ...
    switch (strtolower($arParams['EXTERNAL_TYPE'])) {
        case 'direction':
            $arFilter=array('PARENT_ID'=>$arParams['EXTERNAL_ID']);
            break;
    }
    $arResult['FILTER']=array();
}
