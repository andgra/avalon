...
if($arResult['ELEMENT']['ID']!=''):
	...
	$arResult['EVENT_EDITOR_ID'] = 'contact_'.strval($arParams['ELEMENT_ID']).'_events';
	...
	$arResult['FIELDS'][$arResult['EVENT_TAB_ID']][] = array(
		'id' => 'CONTACT_EVENT',
		'name' => GetMessage('ORDER_FIELD_CONTACT_EVENT'),
		'colspan' => true,
...
$arResult['ORDER_CUSTOM_PAGE_TITLE'] =
	$elementID !=''
	? GetMessage('ORDER_CONTACT_EDIT_TITLE',
		array(
			'#ID#' => $elementID,
			'#FULL_NAME#' => isset($arResult['ELEMENT']['FULL_NAME']) ? $arResult['ELEMENT']['FULL_NAME'] : ''
		)
	)
	: GetMessage('ORDER_CONTACT_CREATE_TITLE');
/*
*/
$formCustomHtml = '<input type="hidden" name="contact_id" value="'.$elementID.'"/>'.$arResult['FORM_CUSTOM_HTML'];
