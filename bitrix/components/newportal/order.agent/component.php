...
$componentPage = '';
$arComponentVariables = array('agent_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_AGENT_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_AGENT_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_AGENT_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['agent_id'];
    ...
    $arResult['PATH_TO_AGENT_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_AGENT_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[agent_id]=#agent_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'AGENT';
