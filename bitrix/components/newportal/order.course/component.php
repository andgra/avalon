...
$componentPage = '';
$arComponentVariables = array('course_id');
...
if ($arParams['SEF_MODE'] == 'Y')
{
    ...
    {
        if(strlen($arParams['PATH_TO_COURSE_'.strToUpper($url)]) <= 0)
            $arResult['PATH_TO_COURSE_'.strToUpper($url)] = $arParams['SEF_FOLDER'].$value;
        else
            $arResult['PATH_TO_COURSE_'.strToUpper($url)] = $arParams['PATH_TO_'.strToUpper($url)];
    }
}
else
{
    $arComponentVariables[] = $arParams['VARIABLE_ALIASES']['course_id'];
    ...
    $arResult['PATH_TO_COURSE_LIST'] = $APPLICATION->GetCurPage();
    $arResult['PATH_TO_COURSE_EDIT'] = $APPLICATION->GetCurPage()."?$arVariableAliases[course_id]=#course_id#&edit";
}
...
$arResult['NAVIGATION_CONTEXT_ID'] = 'COURSE';
