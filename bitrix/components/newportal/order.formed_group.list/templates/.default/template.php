...
?>
    <script type="text/javascript">
        function order_formed_group_delete_grid(title, message, btnTitle, path)
        {
            var d;
            d = new BX.CDialog({
                title: title,
...
$arData=array();
foreach($arResult['FORMED_GROUP'] as $id => $el) {
    $data=array(
        'id' => $id,
        'actions' => array(
            array(
                'ICONCLASS' => 'edit',
                'TITLE' => GetMessage('ORDER_FORMED_GROUP_EDIT_TITLE'),
                'TEXT' => GetMessage('ORDER_FORMED_GROUP_EDIT'),
                'ONCLICK' => 'jsUtils.Redirect([], \''.$el['PATH_TO_FORMED_GROUP_EDIT'].'\');',
                'DEFAULT' => true,
            ),
            /*array(
                'ICONCLASS' => 'delete',
                'TITLE' => GetMessage('ORDER_FORMED_GROUP_DELETE_TITLE'),
                'TEXT' => GetMessage('ORDER_FORMED_GROUP_DELETE'),
                'ONCLICK' => "order_formed_group_delete_grid('".CUtil::JSEscape(GetMessage('ORDER_FORMED_GROUP_DELETE_TITLE'))."', '".CUtil::JSEscape(GetMessage('ORDER_FORMED_GROUP_DELETE_CONFIRM'))."', '".CUtil::JSEscape(GetMessage('ORDER_FORMED_GROUP_DELETE'))."', '".CUtil::JSEscape($el['PATH_TO_FORMED_GROUP_DELETE'])."')"
            )*/
        ),
        'data' => $el,
        'editable' => false
    ...
    $row=array(
        'ID' => '<a href="'.$el['PATH_TO_FORMED_GROUP_EDIT'].'">'.$el['ID'].'</a>',
        'COUNT' => $el['ENROLLED'].' / '.$el['FREE'].' / '.$el['MAX'],
    ...
    if(!$arResult['INTERNAL'] || strtoupper($arResult['EXTERNAL_TYPE'])!='GROUP') {
        $row=array_merge($row,array(
            'GROUP_ID' => ($el['GROUP_ID']!=''?'<a href="'.$el['PATH_TO_GROUP_EDIT'].'" target="_blank">'.$el['GROUP_TITLE'].'</a>':''),
        ));

        if(!$arResult['INTERNAL'] || strtoupper($arResult['EXTERNAL_TYPE'])!='NOMEN') {
            $row=array_merge($row,array(
                'NOMEN_ID' => ($el['NOMEN_ID']!=''?'<a href="'.$el['PATH_TO_NOMEN_EDIT'].'" target="_blank">'.$el['NOMEN_TITLE'].'</a>':''),
            ));
        }
    }
