...
if($arResult['ELEMENT']['ID']!=''):
	...
	$arResult['EVENT_EDITOR_ID'] = 'reg_'.strval($arParams['ELEMENT_ID']).'_events';
	if($arParams['ELEMENT_ID'] !='') {
		ob_start();
		$APPLICATION->IncludeComponent('newportal:order.event.view',
			'',
			array(
				'AJAX_OPTION_ADDITIONAL' => "REG_{$arResult['ELEMENT']['ID']}_EVENT",
				'ENTITY_TYPE' => 'reg',
	...
	$arResult['FIELDS'][$arResult['EVENT_TAB_ID']][] = array(
		'id' => 'REG_EVENT',
		'name' => GetMessage('ORDER_FIELD_REG_EVENT'),
		'colspan' => true,
...
$arResult['ORDER_CUSTOM_PAGE_TITLE'] =
	$elementID > 0
	? GetMessage('ORDER_REG_EDIT_TITLE',
		array(
			'#ID#' => $elementID,
		)
	)
...
/*
*/
$formCustomHtml = '<input type="hidden" name="reg_id" value="'.$elementID.'"/>'.$arResult['FORM_CUSTOM_HTML'];
