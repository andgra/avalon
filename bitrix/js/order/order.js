...
ORDER = function(orderID, div, el, name, element, prefix, multiple, entityType, localize, disableMarkup, params)
{
...
ORDER.Set = function(el, name, subIdName, element, prefix, multiple, entityType, localize, disableMarkup, params)
{
    ...
    obOrder[orderID] = new ORDER(orderID, ORDER.GetWrapperDivPa(el), el, name, element, prefix, multiple,  entityType, localize, disableMarkup, params);
...
ORDER.prototype.Open = function (params)
{
    ...
    var self=this;
    if(BX.type.isNotEmptyString(this.params['serviceUrl']))
    {
        BX.ajax(
            {
                'url': this.params['serviceUrl'],
                'method': 'POST',
                'dataType': 'json',
                'timeout':60,
                'data':
                {
                    'ACTION' : 'GET_ITEMS',
                    'DATA': {'ENTITIES': this.PopupEntityType,'FILTER':this.params['filter']}
                },
                onsuccess: function(data)
                {
                    //console.log(data);
                    self.PopupElement=data;
                    self.PopupItem = {};
                    self.PopupItemSelected = {};
                    for(var i in self.PopupEntityType) {
                        var placeHolder = BX("order-"+self.orderID+"_"+self.name+"-block-"+self.PopupEntityType[i]);
                        if(placeHolder)
                            BX.cleanNode(placeHolder);
                    }
                    for (var i in self.PopupElement)
                        self.PopupAddItem(self.PopupElement[i]);
                    console.log(self)
                },
...
ORDER.PopupSelectItem = function(orderID, element, tab, unsave, select)
{
    ...
    if (tab === null)
    {
        if(obOrder[orderID].PopupTabsIndexId=="order-"+orderID+'_'+obOrder[orderID].name+"-tab-physical")
            blockWrap=BX("order-"+orderID+'_'+obOrder[orderID].name+"-block-physical-selected");
        ...
        if(obOrder[orderID].PopupTabsIndexId=="order-"+orderID+'_'+obOrder[orderID].name+"-tab-agent")
            blockWrap=BX("order-"+orderID+'_'+obOrder[orderID].name+"-block-agent-selected");

        if(obOrder[orderID].PopupTabsIndexId=="order-"+orderID+'_'+obOrder[orderID].name+"-tab-direction")
            blockWrap=BX("order-"+orderID+'_'+obOrder[orderID].name+"-block-direction-selected");
        ...
        if(obOrder[orderID].PopupTabsIndexId=="order-"+orderID+'_'+obOrder[orderID].name+"-tab-formed-group")
            blockWrap=BX("order-"+orderID+'_'+obOrder[orderID].name+"-block-formed-group-selected");
...
ORDER.SearchChange = function(orderID)
{
    ...
    var entityType = obOrder[orderID]['PopupEntityType'][obOrder[orderID]['PopupTabsIndex']];
    ...
    for(var i in obOrder[orderID].PopupElement) {
        if((obOrder[orderID].PopupElement[i]['title'].toLowerCase().search(searchValue.toLowerCase())!=-1
            ...
            || searchDistance(obOrder[orderID].PopupElement[i]['title'].toLowerCase(),searchValue.toLowerCase())<=3
            )
...
ORDER.prototype.PopupCreateValue = function(arElements)
{
    var inputBox = BX("order-"+this.orderID+"_"+this.name+"-input-box");
    var textBox = BX("order-"+this.orderID+"_"+this.name+"-text-box");
    var dataWrap=BX.findParent(this.div,{class: 'order-offer-info-data-wrap'})
    var descBox=BX.findNextSibling(dataWrap);
    /*if(descBox==null) {
        dataWrap.parentNode.insertBefore(BX.create('div',{'props':{'className':'order-offer-info-description'}}), dataWrap.nextSibling);
    }*/
    ...
    for (var type in arElements)
    {
        ...
        for (var i in arElements[type])
        {
            ...
            var addInput=document.createElement('input');
            addInput.type = 'text';
            addInput.name = this.name+'_ENTITY_TYPE';
            addInput.value = type.toLowerCase();
            ...
            if(typeof(arElements[type][i]['advancedInfo']) != "undefined" &&
                arElements[type][i]['advancedInfo'] !== null && descBox!=null) {
                for (var j in arElements[type][i]['advancedInfo']['multiFields']) {
                    var el = arElements[type][i]['advancedInfo']['multiFields'][j];
                    var addDesc = document.createElement('span');
                    if (el['TYPE_ID'] == 'PHONE') {
                        addDesc.className = 'order-offer-info-descrip-tem order-offer-info-descrip-tel';
                        addDesc.appendChild(document.createTextNode("Tel: " + el['VALUE']));
                        var addDescIcon = document.createElement('a');
                        addDescIcon.className = "order-offer-info-descrip-icon";
                        addDescIcon.href = "callto:" + el['VALUE'];
                        addDesc.appendChild(addDescIcon);
                    } else if (el['TYPE_ID'] == 'EMAIL') {
                        addDesc.className = 'order-offer-info-descrip-tem order-offer-info-descrip-email';
                        addDesc.appendChild(document.createTextNode("Email: " + el['VALUE']));
                        var addDescIcon = document.createElement('a');
                        addDescIcon.className = "order-offer-info-descrip-icon";
                        addDescIcon.href = "mailto:" + el['VALUE'];
                        addDesc.appendChild(addDescIcon);
                    }
                ...
                if(typeof(arElements[type][i]['advancedInfo']['contact']) != "undefined" && arElements[type][i]['advancedInfo']['contact'] instanceof Object) {
                    var contactTitle=arElements[type][i]['advancedInfo']['contact']['title'];
                    if(contactTitle!='') {
                        var addDesc = document.createElement('span');
                        addDesc.className = 'order-offer-info-descrip-tem';
                        addDesc.appendChild(document.createTextNode("Contact: " + contactTitle));

                        descBox.appendChild(addDesc);
                        var br = document.createElement('br');
                        descBox.appendChild(br);
                        for (var j in arElements[type][i]['advancedInfo']['contact']['multiFields']) {
                            var el = arElements[type][i]['advancedInfo']['contact']['multiFields'][j];
                            var addDesc = document.createElement('span');
                            if (el['TYPE_ID'] == 'PHONE') {
                                addDesc.className = 'order-offer-info-descrip-tem order-offer-info-descrip-tel';
                                addDesc.appendChild(document.createTextNode("Tel: " + el['VALUE']));
        ...
        if(iTypeEl > 0)
        {
            textBox.appendChild(addOrderLink);
            //rowObject.appendChild(cellObject);
            //tbodyObject.appendChild(rowObject);
            if (this.PopupEntityType.length > 1)
            {
                textBox.appendChild(document.createElement('br'));
                var entityType = document.createElement("font");
                entityType.color = "silver";
                entityType.appendChild(document.createTextNode(this.PopupLocalize[type]));
                textBox.appendChild(entityType);
                /*cellObject = document.createElement("TD");
                 cellObject.className = "field_order_entity_type";
                 cellObject.style.paddingLeft='5px';
...
ORDER.prototype.popupShowMarkup = function()
{
    ...
    var orderBox = this.div.parentNode;
    ...
    for(var i=1; i<=m; i++) {
        for(var j=1; j<=n; j++) {
            d[i][j]=Math.min(
                d[i-1][j]+1,
                d[i][j-1]+1,
                d[i-1][j-1]+Number((s1[j-1]==s2[i-1])?0:1)
            );
            /*console.log(s1[j-1],s2[i-1],'min='+(Math.min(
                d[i-1][j]+1,
                d[i][j-1]+1,
                d[i-1][j-1]+Number((s1[j-1]==s2[i-1])?0:1)
            )),'verh='+(d[i-1][j]+1),'niz='+(d[i][j-1]+1),'prev='+(d[i-1][j-1]),'slag='+((s1[j-1]==s2[i-1])?0:1),'i='+i,'j='+j);*/
        }
    }
    //console.log(d,'!'+s1,'!'+s2,'n='+n,'m='+m);
    return d[m][n];
}
