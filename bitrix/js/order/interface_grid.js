if(typeof(BX.OrderInterfaceGridManager) == 'undefined')
{
	// ownerType, gridId, formName, allRowsCheckBoxId, serviceUrl
	BX.OrderInterfaceGridManager = function()
	{
		this._id = '';
		this._settings = {};
	...
	BX.OrderInterfaceGridManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			BX.addCustomEvent(
				window,
				"OrderInterfaceToolbarMenuShow",
				BX.delegate(this._onToolbarMenuShow, this)
			);
			BX.addCustomEvent(
				window,
				"OrderInterfaceToolbarMenuClose",
		...
		getEditor: function()
		{
			...
			return BX.OrderActivityEditor.items[editorId] ? BX.OrderActivityEditor.items[editorId] : null;
		},
		getServiceUrl: function()
		{
			return this.getSetting('serviceUrl', '/bitrix/components/bitrix/order.activity.editor/ajax.php');
		},
	...
	BX.OrderInterfaceGridManager.items = {};
	BX.OrderInterfaceGridManager.create = function(id, settings)
	{
		var self = new BX.OrderInterfaceGridManager();
	...
	BX.OrderInterfaceGridManager.addEmail = function(managerId, settings)
	{
	...
	BX.OrderInterfaceGridManager.addCall = function(managerId, settings)
	{
		if(typeof(this.items[managerId]) !== 'undefined')
		{
	...
	BX.OrderInterfaceGridManager.addMeeting = function(managerId, settings)
	{
		if(typeof(this.items[managerId]) !== 'undefined')
		{
	...
	BX.OrderInterfaceGridManager.addTask = function(managerId, settings)
	{
		if(typeof(this.items[managerId]) !== 'undefined')
		{
	...
	BX.OrderInterfaceGridManager.viewActivity = function(managerId, id, optopns)
	{
		if(typeof(this.items[managerId]) !== 'undefined')
		{
	...
	BX.OrderInterfaceGridManager.showPopup = function(id, anchor, items)
	{
		BX.PopupMenu.show(
			id,
			anchor,
	...
	BX.OrderInterfaceGridManager.reloadGrid = function(gridId)
	{
	...
	BX.OrderInterfaceGridManager.applyFilter = function(gridId, filterName)
	{
		var grid = window['bxGrid_' + gridId];
		if(!grid || !BX.type.isFunction(grid.Reload))
		{
	...
	BX.OrderInterfaceGridManager.clearFilter = function(gridId)
	{
		var grid = window['bxGrid_' + gridId];
	...
	BX.OrderInterfaceGridManager.menus = {};
	BX.OrderInterfaceGridManager.createMenu = function(menuId, items, zIndex)
	{
	...
	BX.OrderInterfaceGridManager.showMenu = function(menuId, anchor)
	{
	...
	BX.OrderInterfaceGridManager.expandEllipsis = function(ellepsis)
	{
	    ...
	    var cut = BX.findNextSibling(ellepsis, { 'class': 'bx-order-text-cut-on' });
		if(cut)
		{
			BX.removeClass(cut, 'bx-order-text-cut-on');
			BX.addClass(cut, 'bx-order-text-cut-off');
			cut.style.display = '';
		}
