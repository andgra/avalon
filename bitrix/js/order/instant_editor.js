BX.OrderInstantEditor = function ()
{
	this._id = '';
	this._settings = {};
	this._fields = [];
...
BX.OrderInstantEditor.prototype =
{
	initialize: function (id, settings)
	{
		...
		for(var i = 0; i < containerID.length; i++)
		{
			...
			var fieldContainers = BX.findChildren(container, { 'className':'order-instant-editor-fld-block' }, true);
			if(fieldContainers)
			{
		...
		BX.onCustomEvent('OrderInstantEditorCreated', [ this ]);
	},
	getId: function()
	{
		return this._id;
	...
	registerField: function(fieldContainer)
	{
		...
		var button = BX.findChild(fieldContainer, { 'className':'order-instant-editor-fld-btn' }, true, false);
		if(!button)
		{
			return false;
		}
		...
		var field = BX.OrderInstantEditorField.create(fieldContainer, button, this);
	...
	{
		BX.onCustomEvent(this, 'OrderInstantEditorFieldValueSaved', [ name, value ]);
	},
	saveFieldValue: function(name, value, type, callback)
	{
		...
		BX.ajax(
			{
				'url': this.getSetting('url', '/bitrix/components/bitrix/order.deal.show/ajax.php'),
				'method': 'POST',
				'dataType': 'json',
				'data':
				{
				...
				{
					BX.onCustomEvent(self, 'OrderInstantEditorFieldValueSaved', [ name, value ]);
	...
	setFieldValue: function(name, val)
	{
		...
		BX.onCustomEvent(this, 'OrderInstantEditorFieldValueChanged', [ name, val ]);
	},
	setFieldReadOnly: function(name, readonly)
	{
		...
		BX.onCustomEvent('OrderInstantEditorSetFieldReadOnly', [ this, name, readonly ]);
		var fields = this.getFieldsByName(name);
...
BX.OrderInstantEditor._default = null;
BX.OrderInstantEditor.getDefault = function()
{
	return this._default;
};

BX.OrderInstantEditor.items = {};
BX.OrderInstantEditor.create = function(id, settings)
{
	if(!BX.type.isNotEmptyString(id))
	{
		throw 'BX.OrderInstantEditor.create: id is not defined!';
	}

	var self = new BX.OrderInstantEditor();
...
BX.OrderInstantEditorFieldMode =
{
	edit: 1,
	view: 2
};

BX.OrderInstantEditorField = function()
{
	this._mode = BX.OrderInstantEditorFieldMode.view;
...
BX.OrderInstantEditorField.prototype =
{
	initialize:function (container, button, manager)
	{
		if(!container)
		{
			throw "BX.OrderInstantEditorField.initialize: 'container' is not defined!";
		}
		this._container = container;

		if(!button)
		{
			throw "BX.OrderInstantEditorField.initialize: 'button' is not defined!";
		}
		this._button = button;
		button.setAttribute('title', BX.OrderInstantEditorMessages.editButtonTitle);

		if(!manager)
		{
			throw "BX.OrderInstantEditorField.initialize: 'manager' is not defined!";
		}
		...
		if(BX.hasClass(this._button, 'order-instant-editor-fld-btn-input'))
		{
			this._type = 'text';
			fieldViewers = BX.findChild(this._container, { 'className':'order-instant-editor-fld-text' }, true, true);
			fieldSuffixes = BX.findChild(this._container, { 'className':'order-instant-editor-fld-suffix' }, true, true);
		}
		else if(BX.hasClass(this._button, 'order-instant-editor-fld-btn-lhe'))
		{
			this._type = 'lhe';
			fieldViewers = BX.findChild(this._container, { 'className':'order-instant-editor-fld-text' }, true, true);

			var data = {};
			var lheData = BX.findChild(this._container, { 'tagName':'input', 'className':'order-instant-editor-lhe-data' }, true, false);
			if(lheData)
			{
				try
				{
	...
	getFieldID: function()
	{
		var ary = BX.findChild(this._container, { 'className':'order-instant-editor-data-name' }, true, true);
		if(!ary)
		{
			return '';
		}
	...
	processUpdate: function(result)
	{
		...
		if(this._type === 'phone' && this.getCalltoFormat() === BX.OrderCalltoFormat.custom)
		{
	...
	_initFieldValue: function()
	{
		...
		{
			var el = BX.findChild(this._container, { 'className':'order-instant-editor-data-value' }, true, false);
			this._value = el ? el.value : '';
		}
	},
	getCalltoFormat: function()
	{
		return this._manager.getSetting('callToFormat', BX.OrderCalltoFormat.slashless);
	},
	...
	setFieldValue: function(v)
	{
		...
		{
			fieldViewer = BX.findChild(this._container, { 'className':'order-instant-editor-fld-text' }, true, false);
			fieldEditor = BX.findChild(this._container, { 'className':'order-instant-editor-data-input' }, true, false);
			fieldEditor.value = v;
	...
	_setFieldValue: function(val)
	{
		...
		if(!this.isMultiple())
		{
			var f = BX.findChild(this._container, { 'className':'order-instant-editor-data-value' }, true, false);
			if(f)
			{
				f.value = val;
			}
	...
	_handleFieldEditorBlur: function(e)
	{
		...
		if(this._type === 'multitext')
		{
			...
			var fieldEditor = BX.findChild(this._container, { 'className':'order-instant-editor-data-input' }, true, true);
			if(fieldEditor)
			{
	...
	{
		if(this._mode !== BX.OrderInstantEditorFieldMode.edit)
		{
			return;
		}
	...
	_switch2Edit: function(force)
	{
		...
		if(this._isReadOnly || (!force && this._mode === BX.OrderInstantEditorFieldMode.edit))
		{
			return;
		}

		BX.addClass(this._container, 'order-instant-editor-fld-editable');
		...
		this._mode = BX.OrderInstantEditorFieldMode.edit;
	},
	_switch2View: function(force)
	{
		...
		if(!force && this._mode === BX.OrderInstantEditorFieldMode.view)
		{
			return;
		}
		...
		if(fieldViewer)
		{
			if(this._isDisabled)
			{
				BX.addClass(fieldViewer, 'order-instant-editor-fld-disabled');
			}
			else
			{
				BX.removeClass(fieldViewer, 'order-instant-editor-fld-disabled');
			}
		}

		BX.removeClass(this._container, 'order-instant-editor-fld-editable');

		var button = BX.findChild(this._container, { 'className':'order-instant-editor-fld-btn' }, true, false);
		if(button)
		{
			if(this._isReadOnly)
			{
				BX.addClass(this._container, 'order-detail-editable-locked');
				button.setAttribute('title', BX.OrderInstantEditorMessages.lockButtonTitle);
			}
			else
			{
				BX.removeClass(this._container, 'order-detail-editable-locked');
				button.setAttribute('title', BX.OrderInstantEditorMessages.editButtonTitle);
			}
		}
...
BX.OrderInstantEditorField.create = function(container, button, manager)
{
	var self = new BX.OrderInstantEditorField();
	self.initialize(container, button, manager);
	return self;
};


BX.OrderInstantEditorMessages =
{
