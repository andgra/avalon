...
class COrderNomen
{
	static public $sUFEntityID = 'ORDER_NOMEN';
	public $LAST_ERROR = '';
	public $cPerms = null;
	protected $bCheckPermission = true;
	const TABLE_ALIAS = 'L';
	protected static $TYPE_NAME = 'NOMEN';
    ...
    public static function GetFields()
	{
		...
		$directionJoin = ' LEFT JOIN b_order_direction D ON L.DIRECTION_ID = D.ID';
		...
		$result = array(
			...
			'DIRECTION_ID' => array('FIELD' => 'L.DIRECTION_ID', 'TYPE' => 'string'),
			'DIRECTION_TITLE' => array('FIELD' => 'D.TITLE', 'TYPE' => 'string', 'FROM' => $directionJoin),
            
			'SEMESTER' => array('FIELD' => 'L.SEMESTER', 'TYPE' => 'string'),
			'PRICE' => array('FIELD' => 'L.PRICE', 'TYPE' => 'string'),
    ...
    public static function GetListEx($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		$lb->SetUserFields(array('PRICE' => array('MULTIPLE' => 'Y')));
    ...
    public static function CompareFields($arFieldsOrig, $arFieldsModif) {
		...
		if (isset($arFieldsOrig['DIRECTION_ID']) && isset($arFieldsModif['DIRECTION_ID'])
			&& $arFieldsOrig['DIRECTION_ID'] != $arFieldsModif['DIRECTION_ID'])
			$arMsg[] = Array(
				'ENTITY_FIELD' => 'DIRECTION_ID',
				'EVENT_NAME' => GetMessage('ORDER_FIELD_COMPARE_DIRECTION_ID'),
				'VALUE_OLD' => $arFieldsOrig['DIRECTION_ID'],
				'VALUE_NEW' => $arFieldsModif['DIRECTION_ID'],
			);

		if (isset($arFieldsOrig['SEMESTER']) && isset($arFieldsModif['SEMESTER'])
			&& $arFieldsOrig['SEMESTER'] != $arFieldsModif['SEMESTER'])
			$arMsg[] = Array(
				'ENTITY_FIELD' => 'SEMESTER',
				'EVENT_NAME' => GetMessage('ORDER_FIELD_COMPARE_SEMESTER'),
				'VALUE_OLD' => $arFieldsOrig['SEMESTER'],
				'VALUE_NEW' => $arFieldsModif['SEMESTER'],
			);

		if (isset($arFieldsOrig['PRICE']) && isset($arFieldsModif['PRICE'])
			&& $arFieldsOrig['PRICE'] != $arFieldsModif['PRICE']
			&& unserialize($arFieldsOrig['PRICE']) != unserialize($arFieldsModif['PRICE']))
			$arMsg[] = Array(
				'ENTITY_FIELD' => 'PRICE',
				'EVENT_NAME' => GetMessage('ORDER_FIELD_COMPARE_PRICE'),
				'VALUE_OLD' => $arFieldsOrig['PRICE'],
				'VALUE_NEW' => $arFieldsModif['PRICE'],
			);
	...
	public function Update($ID, array &$arFields, $bCompare = true,  $options = array()) {
        ...
        if(isset($arFields['PRICE']) && !is_array(unserialize($arFields['PRICE'])))
            $arFields['PRICE']=serialize(array());

		//$arFields['PRICE']=addslashes($arFields['PRICE']);
		...
		else
		{
			...
			$beforeEvents = GetModuleEvents('order', 'OnBeforeOrderNomenUpdate');
			while ($arEvent = $beforeEvents->Fetch())
			{
				if(ExecuteModuleEventEx($arEvent, array(&$arFields)) === false)
				{
					...
					{
						$this->LAST_ERROR = GetMessage('ORDER_NOMEN_UPDATE_CANCELED', array('#NAME#' => $arEvent['TO_NAME']));
						$arFields['RESULT_MESSAGE'] = &$this->LAST_ERROR;
					}
					return false;
				}
			...
			unset($arFields['ID']);
			$sUpdate = $DB->PrepareUpdate('b_order_nomen', $arFields);
			if (strlen($sUpdate) > 0)
			{
				$DB->Query("UPDATE b_order_nomen SET {$sUpdate} WHERE ID = '{$ID}'", false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
				$bResult = true;
			}
			...
			if($bResult)
			{
				$afterEvents = GetModuleEvents('order', 'OnAfterOrderNomenUpdate');
    ...
    public function Add(array &$arFields, $options = array())
	{
		...
		else
		{
			...
			if($this->bCheckPermission)
			{
				...
				$sEntityPerm = $userPerms->GetPermType('NOMEN', 'ADD');
			...
			$beforeEvents = GetModuleEvents('order', 'OnBeforeOrderNomenAdd');
			while ($arEvent = $beforeEvents->Fetch())
			{
				if(ExecuteModuleEventEx($arEvent, array(&$arFields)) === false)
				{
					...
					{
						$this->LAST_ERROR = GetMessage('ORDER_NOMEN_CREATION_CANCELED', array('#NAME#' => $arEvent['TO_NAME']));
						$arFields['RESULT_MESSAGE'] = &$this->LAST_ERROR;
					}
					return false;
				}
			...
			$DB->Add('b_order_nomen', $arFields, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
			...
			$afterEvents = GetModuleEvents('order', 'OnAfterOrderNomenAdd');
			while ($arEvent = $afterEvents->Fetch())
			{
				ExecuteModuleEventEx($arEvent, array(&$arFields));
			}
			...
			{
				$afterEvents = GetModuleEvents('order', 'OnAfterExternalOrderNomenAdd');
				while ($arEvent = $afterEvents->Fetch())
				{
					ExecuteModuleEventEx($arEvent, array(&$arFields));
				}
    ...
    public function Delete($ID)
	{
		...
		if ($this->bCheckPermission)
		{
			$sEntityPerm = $this->cPerms->GetPermType('NOMEN', 'DELETE');
			if ($sEntityPerm == BX_ORDER_PERM_NONE)
				return false;
		...
		$events = GetModuleEvents('order', 'OnBeforeOrderNomenDelete');
		...
		$dbRes = $DB->Query("DELETE FROM b_order_nomen WHERE ID = '{$ID}'{$sWherePerm}", false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		if (is_object($dbRes) && $dbRes->AffectedRowsCount() > 0)
		{
			...
			$COrderEvent = new COrderEvent();
			$COrderEvent->DeleteByElement('NOMEN', $ID);




			//if(Bitrix\Crm\Settings\HistorySettings::getCurrent()->isDealDeletionEventEnabled())
			COrderEvent::RegisterDeleteEvent(BX_ORDER_NOMEN, $ID, $iUserId, array('FIELDS' => $arFields));

			if(defined("BX_COMP_MANAGED_CACHE"))
			{
				$GLOBALS["CACHE_MANAGER"]->ClearByTag("order_entity_name_".BX_ORDER_NOMEN_ID."_".$ID);
			}

			$afterEvents = GetModuleEvents('order', 'OnAfterOrderNomenDelete');
			while ($arEvent = $afterEvents->Fetch())
			{
