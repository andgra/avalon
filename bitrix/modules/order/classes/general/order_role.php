...
class COrderRole
{
	...
	static public function GetRelation()
	{
		global $DB;
		$sSql = '
			SELECT RR.* FROM b_order_role R, b_order_role_relation RR
	...
	{
		global $DB;
		$sSql = 'DELETE FROM b_order_role_relation';
		...
		foreach ($arRelation as $sRel => $arRole)
		{
			foreach ($arRole as $iRoleID)
			{
				...
				$DB->Add('b_order_role_relation', $arFields, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
			}
		}
	...
	static public function GetRolePerms($ID)
	{
		global $DB;
		$ID = (int)$ID;
		$sSql = 'SELECT * FROM b_order_role_perms WHERE role_id = '.$ID;
		$obRes = $DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$_arResult = array();
	...
	// BX_ORDER_PERM_NONE  - not supported
	static public function GetRoleByAttr($permEntity, $permAttr = COrderPerms::PERM_SELF, $permType = 'READ')
	{
		global $DB;
	...
	static public function GetCalculateRolePermsByRelation($arRel)
	{
		...
		$sSql = "
			SELECT RP.*
			FROM b_order_role_perms RP, b_order_role_relation RR
	...
	static public function GetUserPerms($userID)
	{
		...
		$obRes = $DB->Query(
			"SELECT RP.* FROM b_order_role_perms RP INNER JOIN b_order_role_relation RR ON RR.ROLE_ID = RP.ROLE_ID INNER JOIN b_user_access UA ON UA.ACCESS_CODE = RR.RELATION AND UA.USER_ID = $userID",
			false,
	...
	public function Add(&$arFields)
	{
		...
		else
		{
			...
			$ID = (int)$DB->Add('b_order_role', $arFields, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	protected function SetRoleRelation($ID, $arRelation)
	{
		...
		$sSql = 'DELETE FROM b_order_role_perms WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
				...
				foreach ($arFields as $sField => $arFieldValue)
				{
					if ($sField == '-')
					{
						...
						if ($arFieldValue != '-')
						{
							...
							$DB->Add('b_order_role_perms', $arInsert, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
						}
					}
					else
					{
	...
	public function Update($ID, &$arFields)
	{
		...
		else
		{
			if (!isset($arFields['RELATION']) || !is_array($arFields['RELATION']))
				$arFields['RELATION'] = array();
			$sUpdate = $DB->PrepareUpdate('b_order_role', $arFields, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
			if (strlen($sUpdate) > 0)
				$DB->Query("UPDATE b_order_role SET $sUpdate WHERE ID = $ID", false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	public function Delete($ID)
	{
		global $DB;
		$ID = (int)$ID;
		$sSql = 'DELETE FROM b_order_role_relation WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$sSql = 'DELETE FROM b_order_role_perms WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$sSql = 'DELETE FROM b_order_role WHERE ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	public function CheckFields(&$arFields, $ID = false)
	{
		...
		if (($ID == false || isset($arFields['NAME'])) && empty($arFields['NAME']))
			$this->LAST_ERROR .= GetMessage('ORDER_ERROR_FIELD_IS_MISSING', array('%FIELD_NAME%' => GetMessage('ORDER_FIELD_NAME')))."<br />";
