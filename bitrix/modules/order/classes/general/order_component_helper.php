...
class COrderComponentHelper
{
	...
	public static function SynchronizeFormSettings($formID, $userFieldEntityID, $options = array())
	{
		...
		{
			if(COption::GetOptionString('order', strtolower($formID).'_normalized', 'N') !== 'Y')
			{
				...
				COption::SetOptionString('order', strtolower($formID).'_normalized', 'Y');
			}
		}
	...
	public static function PrepareExportFieldsList(&$arSelect, $arFieldMap, $processMultiFields = true)
	{
		...
		{
			$arMultiFieldTypes = COrderFieldMulti::GetEntityTypes();
...
class COrderInstantEditorHelper
{
	private static $TEMPLATES = array(
		'_LINK_' => '<span class="order-fld-block">
	<span class="order-fld order-fld-#FIELD_TYPE#">
		<a class="order-fld-text" href="#VIEW_VALUE#" target="_blank" >#VALUE#</a>
		<span class="order-fld-value">
			<input class="order-fld-element-input" type="text" value="#VALUE#" />
			<input class="order-fld-element-name" type="hidden" value="#NAME#"/>
		</span>
	</span>
	<span class="order-fld-icon order-fld-icon-#FIELD_TYPE#"></span>
</span>',
		'INPUT' => '<span class="order-fld-block">
	<span class="order-fld order-fld-input">
		<span class="order-fld-text">#VALUE#</span>
		<span class="order-fld-value">
			<input class="order-fld-element-input" type="text" value="#VALUE#" />
			<input class="order-fld-element-name" type="hidden" value="#NAME#"/>
		</span>
	</span>
	<span class="order-fld-icon order-fld-icon-input"></span>
</span>',
		'SELECT' => '<span class="order-fld-block">
	<span class="order-fld order-fld-select">
		<span class="order-fld-text">#TEXT#</span>
		<span class="order-fld-value">
			<select class="order-fld-element-select#CLASS#">#OPTIONS_HTML#</select>
			<input class="order-fld-element-value" type="hidden" value="#VALUE#"/>
			<input class="order-fld-element-name" type="hidden" value="#NAME#"/>
		</span>
	</span>
	<span class="order-fld-icon order-fld-icon-select"></span>
</span>',
		'TEXT_AREA' => '<span class="order-fld-block">
	<span class="order-fld order-fld-textarea">
		<span class="order-fld-text">#VALUE#</span>
		<span class="order-fld-value">
			<textarea class="order-fld-element-textarea" rows="25" cols="50" style="display: none;">#VALUE#</textarea>
			<input class="order-fld-element-name" type="hidden" value="#NAME#"/>
		</span>
	</span>
	<span class="order-fld-icon order-fld-icon-textarea"></span>
</span>',
		'FULL_NAME' => '<span class="order-fld-block order-fld-block-multi-input">
	<span class="order-fld-multi-input">
		<span class="order-fld-multi-input-text">#VALUE_1#</span>
		<input class="order-fld-element-input" type="text" value="#VALUE_1#" style="display: none;"/>
		<input class="order-fld-element-name" type="hidden" value="#NAME_1#"/>
	</span>
	<span class="order-fld-multi-input">
		<span class="order-fld-multi-input-text">#VALUE_2#</span>
		<input class="order-fld-element-input" type="text" value="#VALUE_2#" style="display: none;"/>
		<input class="order-fld-element-name" type="hidden" value="#NAME_2#"/>
	</span>
	<span class="order-fld-multi-input">
		<span class="order-fld-multi-input-text">#VALUE_3#</span>
		<input class="order-fld-element-input" type="text" value="#VALUE_3#" style="display: none;"/>
		<input class="order-fld-element-name" type="hidden" value="#NAME_3#"/>
	</span>
	<span class="order-fld-icon order-fld-icon-multiple-input"></span>
</span>',
		'LHE' => '<span class="order-fld-block">
	<span class="order-fld order-fld-lhe">
		<span class="order-fld-text order-fld-text-lhe">#TEXT#</span>
		<span class="order-fld-value">
			<input class="order-fld-element-value" type="hidden" value="#VALUE#" />
			<input class="order-fld-element-name" type="hidden" value="#NAME#"/>
			<input class="order-fld-element-lhe-data" type="hidden" value="#SETTINGS#"/>
		</span>
	</span>
	<span class="order-fld-icon order-fld-icon-lhe"></span>
</span>
<div id="#WRAPPER_ID#" style="display:none;" class="order-fld-lhe-wrap" >#HTML#</div>'
	);
	...
	public static function CreateMultiFields($fieldTypeID, &$fieldValues, &$formFields, $fieldParams = array(), $readOnlyMode = true)
	{
		...
		foreach($fieldValues as $ID => &$data)
		{
			...
			$field = array(
				'id' => $fieldID,
				'name' => COrderFieldMulti::GetEntityName($fieldTypeID, $valueType, true)
			);

			if($readOnlyMode)
			{
				$field['type'] = 'label';
				$field['value'] = COrderFieldMulti::GetTemplate($fieldTypeID, $valueType, $value);
			}
			else
			{
				$templateType = 'INPUT';
				...
				else
				{
					...
					{
						$viewValue = COrderCallToUrl::Format($value);
					}
	...
	public static function PrepareUserInfo($userID, &$userInfo, $options = array())
	{
		...
		if(IsModuleInstalled('extranet')
			...
			&& $userID != COrderSecurityHelper::GetCurrentUserID()
	...
	public static function PrepareUpdate($ownerTypeID, &$arFields, &$arFieldNames, &$arFieldValues)
	{
		...
		if($ownerTypeID === COrderOwnerType::Lead
			|| $ownerTypeID === COrderOwnerType::Deal
			|| $ownerTypeID === COrderOwnerType::Contact
			|| $ownerTypeID === COrderOwnerType::Company)
		{
